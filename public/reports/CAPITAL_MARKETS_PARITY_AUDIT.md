...
**Date:** 2026-05-05 (recon) · 2026-05-06 (Tier 1 + Tier 2 macro-pair) · 2026-05-07 (Tier 2 fully complete) · 2026-05-08 (gov market_share_pie + Tier 3 positioning items 8 + 9) · 2026-05-09 (Tier 4 #1: Value Proposition Results) · 2026-05-10 (Tier 4 #2: What's New)
**Owner:** Scott Briggs
**Status:** Tier 1 + Tier 2 fully done. Gov market_share_pie live. Tier 3: 2 of 3 positioning items shipped. **Tier 4: 2 of 3 KPI blocks shipped** (`value_proposition_results`, `whatsnew_quarter_kpis`); only `trend_watch_callouts` remains (narrative-driven, needs editorial CMS pattern). Remaining: `nm_cross_asset_class` (deferred, needs RCA broker data) + Trend Watch narrative tiles.
...
| 2026-05-09 | Tier 4 — first `kpi_block` chart_type: `value_proposition_results` | ✅ shipped — establishes the new `chart_type='kpi_block'` pattern. View per vertical returns one row per (period_end × tile) with shape: `tile_id`, `tile_label`, `primary_value`, `primary_format`, `nm_value`, `nm_label`, `non_nm_value`, `non_nm_label`, `sort_order`. Frontend `renderKpiBlock` picks the latest period with populated tiles, renders a tile grid (split-comparison layout when NM/Non-NM populated). Excel `renderKpiBlockTab` writes a compact 4-column summary table. Companion migrations: GovernmentProject `20260509_cm_gov_value_prop_kpis` (NOI from `noi` column), DialysisProject `20260509_cm_dialysis_value_prop_kpis` (NOI proxied from `rent_at_sale`). LCC migration `20260509_cm_chart_catalog_value_prop_kpis` adds the `value_proposition_results` row with `chart_type='kpi_block'`, `applies_to_verticals={gov,dialysis}`, phase=4. Live smoke (gov 2026-Q1): Avg NOI $1.93M, Avg Cap 9.0% (NM 7.5% / Non-NM 9.26%), Avg Price $11.46M (NM $20.0M / Non-NM $11.23M). Dialysis NM splits null (last NM dialysis sale was 2022, outside latest 12-mo TTM — honest data, not a view bug). |
| 2026-05-10 | Tier 4 — second `kpi_block`: `whatsnew_quarter_kpis` (dialysis p.3 front-of-deck) | ✅ shipped — 3 dynamic tiles per quarter (TTM Volume YoY, TTM Cap, 10-Year Treasury) reusing the kpi_block contract from `value_proposition_results`. DialysisProject migration `20260510_cm_dialysis_whatsnew_kpis` joins `cm_dialysis_volume_ttm_q` + `cm_dialysis_market_quarterly` + `cm_dialysis_macro_rates_q`. LCC migration `20260510_cm_chart_catalog_whatsnew_kpis` adds the catalog row + frontend `PHASE_1_TEMPLATES` entry + Excel `TAB_NAMES` entry (no new render code — the existing `renderKpiBlock` / `renderKpiBlockTab` handle any `chart_type='kpi_block'`). Dialysis-only (gov deck doesn't have the same front-of-deck cluster). Live smoke (2025-Q4): TTM Volume YoY +66.1%, TTM Cap 7.70%, 10Y Treasury 4.10%. Tier 4 #4 NHE tile (annual external + dual-figure layout) deferred to a future PR — needs an `external_kpi_values` table or hardcoded year-keyed values. |
