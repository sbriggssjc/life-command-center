...
### Batch 3 (2026-05-29) — Industry Participants + remaining-item triage

| Item | What shipped / status | Where | Verification |
|---|---|---|---|
| **Industry Participants (dialysis p.10)** | Built `cm_dialysis_industry_participants` — live national operator landscape from `medicare_clinics.chain_organization` (top 10 chains by clinic count + "All Remaining Chains" rollup; null→"Unreported"). Replaces the deck's static DaVita-vs-FMC reference table. Ranked DataTable; new `Data_Industry_Part` export tab. | dialysis view + LCC catalog **applied live**; `cm-excel-export.js` CHART_COLUMNS + TAB_NAMES (**needs Vercel deploy**); migrations `DialysisProject/.../20260529150000_cm_dialysis_industry_participants.sql`, `life-command-center/.../20260529150000_cm_chart_catalog_industry_participants.sql` | acorn OK; live snapshot: DaVita 32.8% + Fresenius 32.4% (~65% duopoly), US Renal 4.3%, DCI 2.8% |
| **`nm_cross_asset_class`** | **Confirmed blocked.** `cm_rca_quarterly` has RCA *market* caps by product type, but there is no NM-brokered deal data outside gov/dialysis, so the NM side of the cross-asset comparison (Retail/Office/Industrial/Net Lease) cannot be computed from our DBs. Needs RCA's broker-tagged full subscription. | — | Verified `cm_rca_quarterly` schema (no broker dimension) |
| **`listing_broker` backfill** | **Out of this environment.** Root cause is NULL/first-name `listing_broker` on most sales; the fix is a CoStar/CREXi scraping workstream, which the sandbox's restricted web access can't perform. The entity normalizer from item 2 is available to dedupe broker buckets once the values land. | — | — |
| **Reference: Standard BTS Lease Terms (p.11)** | Left as editorial/`cm_narratives` content (genuinely static, not data-driven). | — | — |
