...
| **11 — Volume boxy** | ✅ | `quarterly_volume_bars` rebuilt as a trailing-3-month rolling SUM of true monthly volume/count (`master_m.monthly_volume`), at monthly grain — moves monthly instead of repeating the quarter total 3×. Headers → "Rolling 3-Mo …". **Hardened 2026-08-13:** the full 39-column `master_m` select intermittently fails PostgREST serialization in prod (Round 6b), which silently dropped the composer to a fallback that re-emitted the boxy quarter-repeat (351 monthly bars, one quarter total each — the exact symptom in the 2026-06-30 gov export). Fix: the export now re-fetches a **slim `(period_end, monthly_volume, monthly_count)` projection** so the rolling path survives that failure, and the last-resort fallback now **collapses to one bar per quarter** instead of the monthly-repeat. Locked by `test/cm-quarterly-volume-bars.test.mjs`. |
