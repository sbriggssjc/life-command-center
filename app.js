...
// ============================================================
// COMMAND QUEUE — strategic/important/urgent prioritized homepage
// ============================================================

function renderCommandQueue() {
  const el = document.getElementById('commandQueueContent');
  if (!el) return;
  ...
  if (!dailyBriefingSnapshot) {
    el.innerHTML = '<div class="widget-error"><div class="err-msg">Command queue unavailable.</div><button class="retry-btn" onclick="loadDailyBriefingData(true)">Retry</button></div>';
    return;
  }

  const snap = dailyBriefingSnapshot;
  const usp = snap.user_specific_priorities || {};
  const priorities = usp.today_priorities || usp.today_top_5 || [];
  const wc = (snap.team_level_production_signals || {}).work_counts || {};
  const inbox = (snap.team_level_production_signals || {}).inbox_summary || {};

  // Group by tier
  const strategic = priorities.filter(p => p.tier === 'strategic');
  const important = priorities.filter(p => p.tier === 'important');
  const urgent = priorities.filter(p => p.tier === 'urgent' || !p.tier);

  // Also include inbox items not already in priorities
  const inboxItems = (inbox.items || []).slice(0, 5);

  let html = '';

  // Quick stats bar
  html += '<div style="display:flex;gap:12px;margin-bottom:14px;flex-wrap:wrap">';
  html += _cqStatPill('Strategic', usp.strategic_count || strategic.length, '#d32f2f');
  html += _cqStatPill('Important', usp.important_count || important.length, '#f57c00');
  html += _cqStatPill('Urgent', usp.urgent_count || urgent.length, 'var(--accent2)');
  html += _cqStatPill('Inbox', (inbox.total_new || 0) + (inbox.total_triaged || 0), 'var(--text3)');
  if (wc.overdue > 0) html += _cqStatPill('Overdue', wc.overdue, '#d32f2f');
  html += '</div>';

  // Strategic section
  if (strategic.length > 0) {
    html += _cqSection('strategic', 'Do First — Revenue & Deal Actions', strategic);
  }

  // Important section
  if (important.length > 0) {
    html += _cqSection('important', 'Do Second — Business Development & Pipeline', important);
  }

  // Urgent section (including unclassified inbox items)
  const urgentItems = [...urgent];
  // Add inbox items not already in priorities
  if (inboxItems.length > 0 && urgentItems.length < 3) {
    inboxItems.forEach(item => {
      if (!urgentItems.find(u => u.id === item.id)) {
        urgentItems.push({
          id: item.id,
          title: item.title || '(untitled)',
          status: item.status,
          priority: item.priority,
          type: item.source_type || 'inbox',
          tier: 'urgent',
          _source: 'inbox',
  ...
  if (urgentItems.length > 0) {
    html += _cqSection('urgent', 'Do Third — Respond & Process', urgentItems.slice(0, 5));
  }

  // Recommended calls
  const calls = usp.recommended_calls || [];
  if (calls.length > 0) {
    html += '<div class="cq-section">';
    html += '<div class="cq-section-header"><span class="cq-section-label">Touchpoints Overdue</span></div>';
    calls.forEach(c => {
      html += '<div class="cq-item tier-important">';
      html += '<div class="cq-item-title">' + esc(c.name || c.title || '') + '</div>';
      html += '<div class="cq-item-context">';
      if (c.company) html += esc(c.company) + ' &middot; ';
      if (c.days_since_touch) html += c.days_since_touch + ' days since last touch &middot; ';
      if (c.score) html += 'Score: ' + c.score;
      html += '</div>';
      html += '<div class="cq-item-actions">';
      html += '<button class="cq-action primary" onclick="sendCopilotAction(\'draft_outreach_email\',{contact_name:\'' + esc(c.name || '').replace(/'/g, "\\'") + '\',intent:\'quarterly touchpoint\'},true)">Draft Email</button>';
      html += '<button class="cq-action" onclick="sendCopilotAction(\'get_relationship_context\',{contact_name:\'' + esc(c.name || '').replace(/'/g, "\\'") + '\'})">Prep Brief</button>';
      html += '</div></div>';
    });
    html += '</div>';
  }

  if (!strategic.length && !important.length && !urgentItems.length && !calls.length) {
    html += '<div class="cq-empty">No prioritized items yet. Sync your Salesforce and Outlook data to populate the command queue.</div>';
  }

  el.innerHTML = html;
}

function _cqStatPill(label, value, color) {
  return '<div style="display:flex;align-items:center;gap:4px;font-size:11px"><span style="width:8px;height:8px;border-radius:50%;background:' + color + ';display:inline-block"></span><span style="color:var(--text3)">' + label + ':</span><strong style="color:var(--text)">' + value + '</strong></div>';
}

function _cqSection(tier, label, items) {
  const tierClass = 'cq-tier-' + tier;
  let html = '<div class="cq-section">';
  html += '<div class="cq-section-header"><span class="cq-tier-badge ' + tierClass + '">' + tier + '</span><span class="cq-section-label">' + esc(label) + '</span></div>';

  items.forEach(item => {
    const title = item.title || '(Untitled)';
    const sender = item.metadata?.sender_email || item.metadata?.sf_who || '';
    const source = item._source || item.source || item.type || '';
    const hasAttachments = item.metadata?.has_attachments;

    html += '<div class="cq-item tier-' + tier + '">';
    html += '<div class="cq-item-title">' + esc(title) + '</div>';

    // Context line
    let ctx = [];
    if (sender) ctx.push('From: ' + esc(sender));
    if (item.domain) ctx.push(esc(item.domain));
    if (item.due_date) ctx.push('Due: ' + item.due_date.split('T')[0]);
    if (hasAttachments) ctx.push(item.metadata.attachment_count + ' attachments');
    if (source) ctx.push(esc(source));
    if (ctx.length) {
      html += '<div class="cq-item-context">' + ctx.join(' &middot; ') + '</div>';
    }

    // Action buttons
    html += '<div class="cq-item-actions">';
    if (item.type === 'flagged_email' || item.type === 'inbox' || item.source_type === 'flagged_email') {
      html += '<a class="cq-action primary" href="/?page=pageMessages">View Email</a>';
      html += '<button class="cq-action" onclick="sendCopilotAction(\'draft_outreach_email\',{contact_name:\'' + esc(sender).replace(/'/g, "\\'") + '\',intent:\'respond to their email about ' + esc(title).replace(/'/g, "\\'").substring(0, 40) + '\'})">Draft Response</button>';
      html += '<button class="cq-action" onclick="sendCopilotAction(\'triage_inbox_item\',{id:\'' + (item.id || '') + '\'})">Triage</button>';
    } else if (item.type === 'sf_activity' || item.type === 'sf_deal') {
      html += '<a class="cq-action primary" href="/?page=pageBiz">View in CRM</a>';
      html += '<button class="cq-action" onclick="sendCopilotAction(\'generate_listing_pursuit_dossier\',{entity_name:\'' + esc(title).replace(/'/g, "\\'").substring(0, 40) + '\'})">Pursuit Dossier</button>';
    } else if (item.type === 'action' || item.type === 'follow_up') {
      html += '<a class="cq-action primary" href="/?tab=queue&id=' + (item.id || '') + '">Open</a>';
      html += '<button class="cq-action" onclick="sendCopilotAction(\'update_execution_task_status\',{id:\'' + (item.id || '') + '\',status:\'in_progress\'})">Start</button>';
    } else {
      html += '<a class="cq-action primary" href="/?tab=inbox&id=' + (item.id || '') + '">View</a>';
    }
    html += '</div>';
    html += '</div>';
  });
...
// BD Cadence Tracker
function renderBdCadence() {
  const el = document.getElementById('bdCadenceContent');
  if (!el) return;

  if (!dailyBriefingSnapshot) {
    el.innerHTML = '<div class="cq-empty">Loading cadence data...</div>';
    return;
  }

  const usp = dailyBriefingSnapshot.user_specific_priorities || {};
  const wc = (dailyBriefingSnapshot.team_level_production_signals || {}).work_counts || {};
  const sf = usp.sf_activity_summary || {};
  const calls = usp.recommended_calls || [];

  // Calculate cadence metrics
  const touchpointsThisWeek = (sf.calls || 0) + (sf.emails || 0);
  const touchpointTarget = 30;
  const callsThisWeek = sf.calls || 0;
  const callTarget = 15;
  const overdueContacts = calls.length;
  const completedThisWeek = wc.completed_week || 0;
  const pipelineDeals = (usp.pipeline_deals || []).length;

  let html = '<div class="bd-grid">';
  html += _bdMetric('BD Touchpoints', touchpointsThisWeek, touchpointTarget, 'var(--accent)');
  html += _bdMetric('Calls', callsThisWeek, callTarget, 'var(--green)');
  html += _bdMetric('Completed', completedThisWeek, 0, 'var(--green)');
  html += _bdMetric('Pipeline Deals', pipelineDeals, 0, '#f57c00');
  html += _bdMetric('Contacts Overdue', overdueContacts, 0, overdueContacts > 3 ? '#d32f2f' : 'var(--text2)');
  html += '</div>';

  el.innerHTML = html;
}

function _bdMetric(label, value, target, color) {
  let html = '<div class="bd-metric">';
  html += '<div class="bd-metric-value" style="color:' + color + '">' + value;
  if (target > 0) html += '<span style="font-size:12px;color:var(--text3);font-weight:400">/' + target + '</span>';
  html += '</div>';
  html += '<div class="bd-metric-label">' + label + '</div>';
  if (target > 0) {
    const pct = Math.min(100, Math.round((value / target) * 100));
    const barColor = pct >= 80 ? 'var(--green)' : pct >= 50 ? '#f57c00' : '#d32f2f';
    html += '<div class="bd-metric-bar"><div class="bd-metric-fill" style="width:' + pct + '%;background:' + barColor + '"></div></div>';
  }
  html += '</div>';
  return html;
}

// Legacy daily briefing panel (kept for backward compatibility)
function renderDailyBriefingPanel() {
  // Render the command queue instead
  renderCommandQueue();
  renderBdCadence();
