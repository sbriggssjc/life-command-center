...
function buildChartConfig(chart, brand) {
  ...
  switch (chart.chart_template_id) {
    ...
    case 'available_by_term_summary': {
      ...
      // (Avg Price, left axis) + 4 dot series (Avg Cap, Upper Q,
      // Median, Lower Q) on right axis.
      // Round 32 split this into 2 charts; Round 33 RESTORED the
      // combined layout per user feedback: "Missing all cap rate
    ...
    case 'cap_rate_ttm_by_quarter': {
      return {
        ...
        // R66b — dia retuned to displayed data 6.48-8.57% -> 6.25-8.75%
        // (R66's 5.75-8.5% clipped the peak); gov stays on CAP_RATE_RANGE.
        options: commonOpts({
          yAxisFormat: AXIS_FORMAT_PERCENT_2DP,
          yAxisRange: (chart.vertical === 'dialysis' ? { min: 0.0625, max: 0.0875 } : CAP_RATE_RANGE),
        }),
      };
    }
    ...
    case 'dom_and_pct_of_ask_monthly': {
      ...
      return {
        ...
        options: (() => {
          ...
          // ~400 -> left 75-450 (was clipping at 300); % of ask bulk 85-96% ->
          // right 82-100% (R66's 70% floor left it flat).
    ...
    case 'seller_sentiment_monthly': {
      ...
      // R66b — dia retuned to displayed last-ask-cap 5.51-7.84% -> 5.25-8.0%
    ...
    case 'available_cap_rate_dot_plot': {
      ...
      // Round 33 — Dia axis title "Firm Lease Term (Years)" → "Lease
      // Term (Years)" per user note: "for dialysis, we don't use firm
      // lease term; just lease term".
    ...
    case 'available_by_firm_term_summary': {
      ...
      // 4 grouped sky-blue bars (Avg Price, left axis) + 4 dot series
      // (Avg Cap, Upper Q, Median, Lower Q) on right axis (4-9%).
      // Cohorts: Sub 5 Year Term / 5-8 Year Term / 8-12 Year Term /
      // 12+ Year Term firm lease years.
      // Round 33 — Restored cap-dispersion dots (R32 had simplified
      // to price-bar-only per a misread of the user note). User: "the
      // cap rate portion of our charts are missing and we want to
      // remove the count from the labels for average deal size/price".
      const termRows = rows || [];
      const termLabels = termRows.map(r => r.term_bucket || '?');
      const dotPointRadius = 6;
      const dotPointStyle = 'rectRot';
    ...
    case 'sold_cap_by_term_dot_plot': {
      ...
      // R66b — retuned to displayed data (2026-06-01): sold dia 5.67-7.87% ->
      // 5.5-8.0% (was ~46% fill); sold gov 6.40-11.81% -> 6.0-12.0% (was
      // clipping the <5yr cohort at 11%); asking dia 4.94-7.80% -> 4.75-8.0%.
      ...
      if (chart.chart_template_id === 'sold_cap_by_term_dot_plot') {
        dotRange = govLikeDot ? { min: 0.0575, max: 0.08 } : { min: 0.055, max: 0.0775 };  // R66q gov ladder 6.07-7.72% (was 6-12%); R66x dia 5.69-7.40%
