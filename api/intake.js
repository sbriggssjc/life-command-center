...
async function handleOutlookSent(req, res) {
  ...
  if (wasNew && (dealEntityId || partyEntityId) && !backfill) {
    // Self-updating to-do engine (outbound): resolve the deal's offer_review +
    // reach-out follow_ups AND, when an offer was just submitted, SCHEDULE the
    // seller follow-up (due +1 day). One call closes what the send satisfies and
