...
--
-- WHY: MERGE1 shipped `_dia_merge_fold_one_row` and `dia_merge_fold_table` as SECURITY DEFINER,
...
--
-- These are DESTRUCTIVE and take a TABLE NAME as a parameter — `dia_merge_fold_table` runs
...
-- `dia_merge_property_reversible` three days earlier. `dia_merge_property`, their only caller,
...
begin
  for r in
    ...
    where n.nspname = 'public'
      and p.proname in ('_dia_merge_fold_one_row', 'dia_merge_fold_table')
  loop
...
begin
  ...
  if v_bad > 0 then
    raise exception 'MERGE1-sec: % dia fold function(s) still reachable by anon/authenticated', v_bad;
  end if;
  ...
  if v_svc <> 2 then
    raise exception 'MERGE1-sec: expected 2 dia fold functions executable by service_role, found %', v_svc;
  end if;
end $$;
