-- ============================================================================
-- Dia — v_sjc_broker_contact_ids: live broker-Id list for Flow 6 self-maintenance
--
-- Target: dialysis Supabase (DIA_SUPABASE_URL / zqzrriwuavgrquhisnoa)
-- APPLIED LIVE 2026-05-29 (read-only view, non-breaking).
--
-- Power Automate "Flow 6: SJC Broker Contact Sync" GETs this view via PostgREST
--   GET /rest/v1/v_sjc_broker_contact_ids?select=sf_contact_id
-- to refresh the Contact SOQL IN-list without editing the flow — so new brokers
-- are picked up automatically. Excludes the a1s* junction-object Ids (not Contacts).
-- ============================================================================
...
COMMENT ON VIEW public.v_sjc_broker_contact_ids IS
  'Distinct SJC broker Salesforce Contact Ids (003*) referenced on deals, across the five *_sjc__c broker lookup fields. Source for Power Automate Flow 6 (SJC Broker Contact Sync). Excludes a1s* junction-object Ids.';
