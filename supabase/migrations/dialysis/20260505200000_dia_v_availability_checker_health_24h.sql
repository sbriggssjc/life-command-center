-- ============================================================================
-- Round 76ej.h (dia) — view of the last 24h of availability-checker runs.
--
-- Triage / monitoring surface for the new lcc-availability-checker cron
-- (Round 76ej.g). One row per check_result with a count and a sample of
-- the URLs that produced it. Pairs with the bot-block alert in LCC Opps:
-- when the alert fires, the on-call hops to this view to see WHICH
-- listings are tripping the unreachable verdicts.
-- ============================================================================
...
COMMENT ON VIEW public.v_availability_checker_health_24h IS
  'Round 76ej.h — last 24h breakdown of availability-checker (Round
   76ej.g) outcomes from listing_verification_history. Use to triage a
   bot-block alert: SELECT * FROM v_availability_checker_health_24h
   WHERE check_result IN (''unreachable'',''manual_review_needed'').';
