-- Prompt 31 (dia) - property-record consolidation + multi-source same-event sales.
--
...
COMMENT ON TABLE public.p31_property_consolidation_log IS
  'Prompt 31 reversible ledger for dia property-record consolidation. Stores keep/drop property snapshots and merge evidence before calling dia_merge_property.';

CREATE OR REPLACE VIEW public.v_p31_property_consolidation_groups AS
WITH prop AS (
  SELECT
    p.*,
    public.dia_normalize_state(p.state::text) AS norm_state,
    public.dia_normalize_address(p.address) AS norm_address,
    lower(NULLIF(btrim(coalesce(p.chain_canonical, p.operator, p.tenant, '')), '')) AS operator_key
...
grp AS (
  SELECT
    ...
    count(*) FILTER (WHERE operator_key IS NULL) AS null_operator_count,
    count(DISTINCT operator_key) FILTER (WHERE operator_key IS NOT NULL) AS distinct_operator_count,
    count(DISTINCT medicare_id) FILTER (WHERE medicare_id IS NOT NULL) AS distinct_medicare_count
  FROM prop
...
WITH candidates AS (
  SELECT
    ...
    (
      ...
      (p.building_size IS NOT NULL)::int +
      (p.tenant IS NOT NULL)::int +
      (p.operator IS NOT NULL)::int +
      (p.medicare_id IS NOT NULL)::int +
  ...
  JOIN public.properties p
    ON public.dia_normalize_state(p.state::text) = g.norm_state
   AND public.dia_normalize_address(p.address) = g.norm_address
),
ranked AS (
  SELECT
    c.*,
    ...
    CASE
      WHEN property_count <= 4
       AND null_operator_count = 0
       AND distinct_operator_count <= 1
       AND distinct_medicare_count <= 1 THEN 0.98
      WHEN property_count <= 4
       AND distinct_operator_count <= 1
       AND distinct_medicare_count <= 1 THEN 0.90
      ELSE 0.70
    END AS confidence,
    CASE
      WHEN property_count <= 4
       AND null_operator_count = 0
       AND distinct_operator_count <= 1
       AND distinct_medicare_count <= 1 THEN 'auto_merge'
      ELSE 'review'
    END AS lane
  FROM candidates c
  WINDOW w AS (
...
SELECT
  ...
  jsonb_build_object(
    ...
    'null_operator_count', null_operator_count,
    'distinct_medicare_count', distinct_medicare_count,
    'drop_address', address,
    'drop_city', city,
    'drop_state', state,
    'drop_tenant', tenant,
    'drop_operator', coalesce(chain_canonical, operator),
    'drop_medicare_id', medicare_id,
...
COMMENT ON VIEW public.v_p31_property_consolidation_plan IS
  'Prompt 31 dry-run plan for dia same-address/different-property_id consolidation. lane=auto_merge is conservative; lane=review is not applied by the RPC.';
...
DECLARE
  v_batch text := coalesce(NULLIF(p_batch_tag, ''), 'p31_dia_property_consolidation_' || to_char(now(), 'YYYYMMDD_HH24MISS'));
...
BEGIN
  ...
  LOOP
    ...
    v_result := public.dia_merge_property(v_rec.keep_id, v_rec.drop_id);
...
COMMENT ON FUNCTION public.p31_property_consolidation_apply(boolean, text) IS
  'Prompt 31 dia property consolidation RPC. Dry-run by default. On apply, backs up keep/drop property rows to p31_property_consolidation_log, then calls dia_merge_property for high-confidence lane only.';
...
DECLARE
  v_norm_address text := public.dia_normalize_address(p_address);
  v_norm_state text := public.dia_normalize_state(p_state);
  v_matches jsonb;
  v_count integer;
BEGIN
  ...
  WITH matched AS (
    SELECT p.property_id, p.address, p.city, p.state, p.tenant, p.operator, p.medicare_id
    FROM public.properties p
    WHERE public.dia_normalize_address(p.address) = v_norm_address
      AND (v_norm_state IS NULL OR v_norm_state = '' OR public.dia_normalize_state(p.state::text) = v_norm_state)
...
COMMENT ON FUNCTION public.p31_find_existing_property_by_address(text, text, text) IS
  'Prompt 31 recurrence guard for dia writers: resolve by database-normalized address/state before inserting a new property. Multiple matches return ambiguous so callers do not create another duplicate.';
...
COMMENT ON TABLE public.p31_same_event_sale_reconciliation_log IS
  'Prompt 31 reversible ledger for dia multi-source same-event sale demotions. Reverse by setting logged loser_sale_id rows back to transaction_state=live and dedup_group_id=null.';
...
WITH base AS (
  SELECT
    ...
    s.updated_at,
    lower(regexp_replace(coalesce(s.buyer_name, ''), '[^a-z0-9]', '', 'g')) AS buyer_key,
    lower(regexp_replace(coalesce(s.seller_name, ''), '[^a-z0-9]', '', 'g')) AS seller_key,
    CASE
      WHEN coalesce(s.cap_rate_quality,'') ~ 'implausible' THEN 4
      WHEN s.cap_rate_source = 'master_curated'
        OR coalesce(s.cap_rate_quality,'') IN ('validated','cmbs_audited','om_actual','om_confirmed','deed_verified','confirmed','lease_confirmed') THEN 1
    ...
    CASE
      ...
      ELSE 10
    END AS source_rank,
    CASE s.cap_rate_confidence WHEN 'high' THEN 1 WHEN 'medium' THEN 2 WHEN 'low' THEN 3 WHEN 'suspect' THEN 4 ELSE 5 END AS conf_rank
...
pairs AS (
  SELECT
    ...
    jsonb_build_object(
      ...
      'loser_buyer', b.buyer_name,
      'survivor_buyer', a.buyer_name,
      'loser_seller', b.seller_name,
      'survivor_seller', a.seller_name,
    ...
    row_number() OVER (
      ...
      ORDER BY a.quality_rank, a.source_rank, a.conf_rank, a.updated_at DESC NULLS LAST, a.sale_id
    ) AS choice_rank
  FROM base b
  JOIN base a
   ...
   AND (
      ...
      OR (a.quality_rank = b.quality_rank AND a.source_rank = b.source_rank AND a.conf_rank = b.conf_rank AND a.updated_at IS NOT DISTINCT FROM b.updated_at AND a.sale_id < b.sale_id)
   )
)
SELECT
  p.*,
...
COMMENT ON VIEW public.v_p31_same_event_sale_plan IS
  'Prompt 31 dry-run plan for dia multi-source same-event sale reconciliation. Conservative: same property, same normalized buyer, different source, <=120 days, <=3% or $1k price delta. Repeat sales outside this window remain distinct.';

CREATE OR REPLACE VIEW public.v_p31_same_event_sale_review AS
WITH live AS (
  SELECT
    property_id,
    lower(regexp_replace(coalesce(buyer_name, ''), '[^a-z0-9]', '', 'g')) AS buyer_key,
    count(*) AS row_count,
    ...
    array_agg(sale_id ORDER BY sale_date, sale_id) AS sale_ids
...
LANGUAGE plpgsql SECURITY DEFINER AS $$
DECLARE
  v_batch text := coalesce(NULLIF(p_batch_tag, ''), 'p31_dia_same_event_sales_' || to_char(now(), 'YYYYMMDD_HH24MISS'));
...
COMMENT ON FUNCTION public.p31_same_event_sales_apply(boolean, text) IS
  'Prompt 31 dia same-event sale reconciliation RPC. Dry-run by default. On apply, soft-demotes multi-source same-event losers to duplicate_superseded and logs every loser.';
...
SELECT
  ...
  s.data_source,
  s.transaction_state,
  row_number() OVER (PARTITION BY s.property_id ORDER BY s.sale_date DESC NULLS LAST, s.sale_id DESC) AS recency_rank
FROM public.sales_transactions s
WHERE s.transaction_state = 'live'
  AND s.property_id IS NOT NULL
...
COMMENT ON VIEW public.v_p31_sale_history_live IS
  'Prompt 31 sale-history view for dia: all live sale rows remain available, with recency_rank=1 identifying the most recent sale per property for appraisal-style pulls.';
...
WITH ordered AS (
  SELECT
    ...
    sale_date,
    sold_price,
    lag(sale_date) OVER (PARTITION BY property_id ORDER BY sale_date, sale_id) AS prev_sale_date,
    lag(sold_price) OVER (PARTITION BY property_id ORDER BY sale_date, sale_id) AS prev_sold_price
  FROM public.sales_transactions
  WHERE transaction_state = 'live'
    AND property_id IS NOT NULL
    AND sale_date IS NOT NULL
...
COMMENT ON VIEW public.v_p31_repeat_sale_census IS
  'Prompt 31 verification view for dia: repeat sales over one year apart are classified as repeat_sale_keep, not dedup candidates.';
