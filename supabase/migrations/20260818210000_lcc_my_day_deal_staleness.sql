-- lcc_my_day: enrich Active Deals with last-touch context (from v_activity_unified) and a
-- stage-aware "stale / going cold" flag, so My Day surfaces which in-motion deals need a
-- touch. Stale thresholds: non_refundable/loi_executed >7d, listing_signed/off_market >14d,
-- bov >10d, or no logged activity at all. Stale deals sort first; adds active_deal_stale
-- count. APPLIED LIVE 2026-07-31.
...
as $function$
  ...
  deals0 as (
    select b.id, b.entity_id, coalesce(nullif(b.deal_name,''), b.property_address, e.name) as name,
           b.stage, b.amount, b.expected_close_date, b.vertical,
  ...
  q0 as (
    select q.entity_id, q.name, q.priority_band, q.reason, q.days_overdue,
           q.last_touch_at, q.last_touch_type, q.effective_owner_role,
           q.current_annual_rent_total, q.rank_annual_rent, q.effective_domain,
  ...
  select jsonb_build_object(
    ...
    'active_deals', coalesce((select jsonb_agg(jsonb_build_object(
        ...
        'vertical',vertical,'unassigned',(eff_owner is null),
        'last_touch_at',last_touch_at,'last_touch',last_title,'last_channel',last_channel,
        'days_since_touch',days_since_touch,'stale',is_stale)
        order by is_stale desc, stage_rank asc, expected_close_date asc nulls last)
      from (select * from deals0 where eff_owner is null or eff_owner=(select uid from me)
            order by stage_rank asc, expected_close_date asc nulls last, amount desc nulls last
