-- Incident fix (2026-07-19): v_priority_queue_live degraded to >60s and saturated
-- LCC Opps connections (auth DB -> forced reset). Root cause: the graph CTEs
-- (connected_entities / person_connected_entities / self_contactable_person_entities)
-- scanned the un-indexable `entity_effective_role` CTE. As entities grew to ~45.8k
-- (SF campaign seed +6.5k), the planner mis-estimated the person index scan at rows=1
-- and chose a Nested Loop over the 43.6k-row CTE scan (~10.5k x 43.6k ~= 450M iters).
-- Fix: those three predicate CTEs now read the base `entities` table directly
-- (semantically identical: entity_effective_role = entities WHERE merged_into IS NULL),
-- so the planner uses hashed subplans / index scans. Band membership is BYTE-IDENTICAL
-- (verified: all 10 bands, 1,122 rows, matching md5 signatures). P-CONTACT branch:
-- >55s -> 271ms; full view >60s -> 1.7s; refresh ~62s -> 1.45s. No band logic changed.
-- Reversible: restore the prior view body.
CREATE OR REPLACE VIEW public.v_priority_queue_live AS
 WITH entity_effective_role AS (
         ...
         -- INCIDENT FIX: read base `entities` directly (not the entity_effective_role
         -- CTE) so the planner uses index/hash scans instead of a nested loop over a
         -- non-indexable CTE scan. Predicate is identical.
         SELECT e0.id AS entity_id
           FROM entities e0
          WHERE e0.merged_into_entity_id IS NULL
            AND ((EXISTS ( SELECT 1
         ...
         -- INCIDENT FIX: same repoint to base `entities`.
         SELECT e0.id AS entity_id
           FROM entities e0
          WHERE e0.merged_into_entity_id IS NULL
            AND ((EXISTS ( SELECT 1
         ...
         -- INCIDENT FIX: read base `entities` directly. This was the primary blowup
         -- (the planner's rows=1 misestimate on the person index scan chose a nested
         -- loop over the 43.6k-row entity_effective_role CTE scan).
