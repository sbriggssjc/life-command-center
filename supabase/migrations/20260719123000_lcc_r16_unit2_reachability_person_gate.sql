-- ============================================================================
-- R16 Unit 2 — reachability gate: SF account identity is NOT a reachable human
-- ----------------------------------------------------------------------------
-- R10 Unit 3b counted an entity as "reachable" (cadence stays in P0/P6/P7) if it
-- had a cadence contact id OR was "connected" — where connected included a bare
-- Salesforce ACCOUNT identity. Grounded live 2026-06-13: of the 395 contactless
-- prospecting cadences, 67 carry an SF account identity but a person CONTACT for
-- 0 of them — so those 67 sat in the OUTREACH bands with NO actual person to
-- email (a dishonest "draft email" card).
--
-- This tightens reachability to a real human: a cadence is reachable iff it
-- carries sf_contact_id / contact_id OR the entity has a relationship to a
-- person-typed entity. A bare SF ACCOUNT identity (external_identities
-- source_system='salesforce') no longer counts. SF-mapped-but-contactless
-- entities therefore correctly sit in P-CONTACT until the R16 Unit 1
-- contact-acquisition worker (or a human) gives them a contact.
--
-- IMPORTANT — scope: this changes ONLY the cadence reachability predicate
-- (P0/P6/P7 vs P-CONTACT). The P0.4 / P0.5 ownership-resolution gates keep using
-- the original `connected_entities` (SF account identity legitimately counts as
-- "connected" for the R6 ownership-resolution question — a different question
-- from "is there a human to email"). So `connected_entities` is left intact and
-- a new `person_connected_entities` CTE (the two person-relationship branches,
-- minus the SF-identity branch) drives reachability.
--
-- DEPLOY ORDER: apply AFTER running the Unit 1 contact-acquisition drain on the
-- 67 SF-mapped set, so most acquire a contact and stay reachable instead of all
-- 67 dumping into P-CONTACT before the pull gets a chance. Applying earlier is
-- still SAFE (P-CONTACT IS the honest state for a contactless entity) — it just
-- shows more manual contact-acquisition work until the worker drains.
--
-- Cache-or-live safe: rewrites v_priority_queue_live (column shape unchanged —
-- only a CTE + the reachable_cadence WHERE changed) and refreshes the queue
-- cache at the end. Apply on LCC Opps (xengecqvemvfknjvbvrq).
-- ============================================================================
...
CREATE OR REPLACE VIEW public.v_priority_queue_live AS
 WITH entity_effective_role AS (
         ...
         -- R16 Unit 2: a cadence is reachable if it carries a contact id OR its
         -- entity has a person-contact relationship. A bare SF account identity
         -- no longer counts (that entity falls to P-CONTACT until Unit 1's
         -- contact-acquisition worker, or a human, attaches a person).
         SELECT cs.entity_id
