...
-- Domain: government (scknotsqkcheojiaewwh)
--
...
--
-- REVERSAL:
--   drop view if exists public.v_gov_deed_ocr_provenance;
--   drop function if exists public.gov_merge_document_extracted_data(bigint, jsonb, text, boolean);
-- ============================================================================

-- ── 1. The single merge owner ───────────────────────────────────────────────
create or replace function public.gov_merge_document_extracted_data(
...
comment on function public.gov_merge_document_extracted_data(bigint, jsonb, text, boolean) is
...
revoke all on function public.gov_merge_document_extracted_data(bigint, jsonb, text, boolean) from public, anon, authenticated;
grant execute on function public.gov_merge_document_extracted_data(bigint, jsonb, text, boolean) to service_role;
...
create or replace view public.v_gov_deed_ocr_provenance as
with deeds as (
  select
    document_id,
...
comment on view public.v_gov_deed_ocr_provenance is
...
revoke all on public.v_gov_deed_ocr_provenance from anon, authenticated;
grant select on public.v_gov_deed_ocr_provenance to service_role;
