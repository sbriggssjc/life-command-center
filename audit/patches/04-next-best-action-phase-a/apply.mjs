#!/usr/bin/env node
// ============================================================================
// LCC Audit Sprint — Item #4 Phase A: propagation gap views + v_next_best_action
// Closes: B-1 (partial), B-3 (partial), B-13 — the dia side of the unified
// ranked gap queue. Phase B = gov mirror + LCC Opps view + backend endpoint
// + Home rail UI.
...
// ============================================================================
// FILE 1: supabase/migrations/dialysis/20260517190000_dia_next_best_action_views_phase_a.sql
// ============================================================================
async function writeMigration(report) {
  const path = resolve(REPO_ROOT, 'supabase', 'migrations', 'dialysis', '20260517190000_dia_next_best_action_views_phase_a.sql');
...
`;
  if (DRY) {
    report.push(['supabase/migrations/dialysis/20260517190000_dia_next_best_action_views_phase_a.sql', SQL.length, 'dry-run (would create)']);
    return;
  }
  await mkdir(dirname(path), { recursive: true });
  await writeFile(path, SQL, 'utf8');
  report.push(['supabase/migrations/dialysis/20260517190000_dia_next_best_action_views_phase_a.sql', SQL.length, 'written']);
}
...
async function updateAuditProgress(report) {
  ...
  // Item #4 row: PENDING → IN PROGRESS (Phase A on dia landed)
  const oldRow = N(`| 4 | Build \`v_next_best_action\` UNION view + Home rail | \`audit/04-next-best-action\` | 🟦 PENDING | B-1, B-3, B-13 | CRITICAL |`);
  const newRow = N(`| 4 | Build \`v_next_best_action\` UNION view + Home rail | \`audit/04-next-best-action-phase-a\` | 🟨 IN PROGRESS | B-1, B-3 (dia), B-13 | CRITICAL · Phase A: 6 gap sources + 3 propagation views on dia (21k+ ranked gaps live). Phase B: gov mirror + LCC Opps view + Home rail UI. |`);
  const n = c.split(oldRow).length - 1;
  if (n === 1) c = c.replace(oldRow, newRow);
  else console.warn('[audit_progress] item-4 row not found (n=' + n + ')');
...
async function main() {
  console.log(`\n=== LCC Audit Sprint — Item #4 Phase A: v_next_best_action (dia) ===`);
  ...
  if (DRY) {
    ...
    console.log(`  node audit/patches/04-next-best-action-phase-a/apply.mjs --apply\n`);
  } else {
    console.log(`\n✓ Apply complete.\n`);
    console.log(`  git add -A && git commit -F audit/patches/04-next-best-action-phase-a/COMMIT_MSG.txt\n`);
  }
}
