...
//
// User-visible payoff for the entire Item #4 build:
//   - Phase A   (dia v_next_best_action view)               shipped 2026-05-17
//   - Phase B-1 (gov mirror)                                shipped 2026-05-17
//   - Phase B-2 (cross-domain endpoint /api/admin)          shipped 2026-05-17
//   - v3       (NOI/cap_rate valuation)                     shipped 2026-05-17
//   - v3.2     (dedupe + junk filter)                       shipped 2026-05-17
//   - Phase C  (Home rail UI)                               ← THIS PATCH
//
// Edits:
//   - index.html        widget block after Home stats grid
//   - styles.css        .nba-* layout + severity colors
//   - app.js            state vars, handlePageLoad, render/load fns,
...
async function replaceUnique(path, oldStr, newStr, report, label) {
  ...
  // Use a function callback so JS does NOT expand replacement patterns
  // (the literal dollar-sign chars in formatNbaValue would otherwise be
  // mistaken for backref tokens and corrupt the output).
...
// TEAM PULSE`,
  ...
  // 5. startAutoRefresh interval — add NBA refresh (anchor: loadHealth();)
  await replaceUnique(path,
    `    loadHealth();
    loadWeather();
    loadMarket();
  ...
  // 6. visibilitychange handler — add NBA refresh (anchor: 6-space indent)
  await replaceUnique(path,
    `      loadWeather();
      loadMarket();
      loadCanonicalData();
