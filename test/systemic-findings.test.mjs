...
describe('assembleReport + month-over-month deltas', () => {
  ...
  it('assembles 10 sections + flattened findings + severity totals', () => {
    const r = assembleReport(inputs, { period: '2026-08', now: '2026-08-07T00:00:00Z' });
    assert.equal(r.sections.length, 10); // +1: W9.5 connectedness
    assert.equal(r.totals.sections, 10);
...
describe('Prompt 73 — per-section resilience (section_errors)', () => {
  ...
  it('a throwing section contributes {section, error} and does NOT kill the tick', () => {
    ...
    // The other 9 sections still built (8 siblings + W9.5 connectedness).
