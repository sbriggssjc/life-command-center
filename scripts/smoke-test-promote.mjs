...
//   2. /api/intake?_route=promote with a known stalled intake_id (NO writes
//      if SAFE=1 is set; just dry-checks the response shape)
//   3. Print a green/red summary
//
// Reads from env (LCC_API_KEY + LCC_BASE_URL + LCC_WORKSPACE_ID), so you can
// invoke it via:
//
//   # If you already have .env.local populated:
//   set -a && source .env.local && set +a && node scripts/smoke-test-promote.mjs
//
//   # Or pull fresh from Vercel (requires vercel CLI logged in to the project):
//   vercel env pull .env.smoke-test --environment=production
//   set -a && source .env.smoke-test && set +a && node scripts/smoke-test-promote.mjs
//   rm .env.smoke-test
//
//   # By default the smoke test DOES the promote (writes to dia DB).
//   # To validate auth+columns without any writes, use SAFE=1:
...
function check(name, ok, detail = '') {
  if (ok) { console.log(`  ${GREEN('✓')} ${name}${detail ? DIM(' — ' + detail) : ''}`); pass++; }
  else    { console.log(`  ${RED('✗')} ${name}${detail ? ' — ' + detail : ''}`);       fail++; fails.push({ name, detail }); }
}
...
if (SAFE) {
  ...
  try {
    ...
    let body; try { body = JSON.parse(text); } catch { body = { raw: text }; }
    check('HTTP 200',           r.status === 200,            `got HTTP ${r.status}`);
    check('No "Intake item not found" error',
                                 body?.error !== 'Intake item not found',
                                 body?.error || 'no error');
    check('Returned ok=true',    body?.ok === true,           `body.ok=${body?.ok}`);
    check('Has domain match',    body?.domain === 'dialysis' || body?.domain === 'government',
                                 `domain=${body?.domain || '?'} property_id=${body?.domain_property_id || '?'}`);
    check('Propagated to domain', body?.propagated === true,  `propagated=${body?.propagated}`);
    if (body?.pipeline_summary) {
      const ps = body.pipeline_summary;
      console.log(`  ${DIM('pipeline summary: contacts=' + (ps.contacts_created ?? '?') +
                            ' tenant=' + (ps.tenant_created ?? '?') +
                            ' sales=' + (ps.sales_recorded ?? '?') +
                            ' domain=' + (ps.domain || '?') + ')')}`);
    }
  } catch (err) {
