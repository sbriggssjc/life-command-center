...
| **CoStar sidebar / public records (PR5/PRI)** | PR5d, PR-scanner-3, PRI2–PRI6, HCRIS-TIMEOUT, HCRIS-TRACKER-BLIND, HCRIS-QIP-DEFICIENCY-TIMEOUT-PATTERN | 2026-09-16 | **Older narrative (HCRIS-TIMEOUT rounds 1-10, PRI6 through the DIA-PROPAGATOR1 tangent, 2026-09-16 to 2026-09-22): [`docs/history/STATUS_open-threads_PR5-PRI-row_to_2026-09-22.md`](../history/STATUS_open-threads_PR5-PRI-row_to_2026-09-22.md).** **👤 2026-09-22, same day: answered — `clinic_quality_metrics` half fixed (PR #7424, Scott reports merged, live proof pending), `ratings` half reopened at 🔴 after this session found CC's "already fixed by RATINGS2" reasoning doesn't hold against live evidence (`ratings` still erroring daily through today, `RATINGS-INSERT-COLLISION`'s fix never durably held). **👤 2026-09-22, round 2 answered: `ratings`' real root cause found and fixed (PR #7425, Scott reports merged) — a client-wide `Prefer: return=minimal` header was silently defeating every UPDATE-response check, not a capped prefetch or partial-index issue. September's "8-hour clean window" was also re-explained: no pipeline run occurred in that window at all, not a fix holding temporarily.** `ratings` closed to 🟡 pending two consecutive clean daily runs (not one). Live proof still owed on all three PRs (`HCRIS-TIMEOUT-10`/#7423, CQM/#7424, `ratings`/#7425) — no run has started on any of their merges yet. See the dated entry below.** **👤 2026-09-23: first post-merge run is a split verdict.** `ratings` is genuinely clean for the first time in this entire saga (0 errors, all 7,013 rows fresh). `clinic_quality_metrics` is still completely broken — same symptom, new mechanism: PR #7424's own `.upsert()` call throws a real `23505` on the very first row it processes, every run, zero writes landing at all. `HCRIS-TIMEOUT-10` also looks good so far (`properties` ~90% of fleet). Separately, `ingestion_tracker` rows are not reliably closing even when Railway shows a run "completed" — a new run started before the prior one's rows ever got a `finish_run()`. See the dated entry below. **👤 2026-09-23, round 3 answered: CQM's real root cause found — same defect class as `ratings`' round-2 fix (a client-wide header silently defeating the upsert).** Fix pushed, PR #7426, Scott reports merged; live proof still pending (the run in progress predates the merge). Two new follow-up items filed: the same header bug likely affects 133 other `.upsert()` call sites repo-wide, and the tracker-never-closes gap has its own filing now. **👤 2026-09-24: `CMS-PIPELINE-STAGE-STARVATION` answered — same header-defect family found a fourth time (on `ingestion_tracker.start_run()`'s own INSERT this time), which turns out to be the real cause of `RATINGS-CQM-CB3-tracker-close` too (the "hard kill" theory filed for that item was wrong, now corrected). Separately, my own prior read of `properties` "decelerating" was also wrong — CC found the write rate was flat all along; the real long pole is `backfill_financials` unconditionally rewriting all 189,851 `facility_patient_counts` rows every run (~9h) and swallowing its own step timeout, starving everything scheduled after it. Fix pushed, PR #7427, Scott reports merged; no live run has tested it yet — see the dated entry below and `PLANNED-BACKLOG.md`'s `CMS-PIPELINE-STAGE-STARVATION` and `RATINGS-CQM-CB3-tracker-close` rows. **👤 2026-09-24, same day: live test confirms #7427 works — first self-closing run ever (1h14m vs. 15-24h), `facility_deficiencies`/`qip_scores` both writing fresh for the first time since 2026-05-16.** Five gaps found in that same run (staged-intake queue not draining, `clinic_financial_estimates` zero writes, `medicare_ingestion` still frozen, run closed `partial` not `success`, tracker `notes` possibly cross-contaminated between rows) plus a new table (`investment_targets`) hit by the CB3 circuit-breaker class — filed as `CMS-PIPELINE-STAGE-STARVATION-2`. See the dated entry below. **👤 2026-09-24, round 2 answered same day: 3 of 5 gaps were real (and fixed), 2 were correct behavior misread as bugs; `ratings` closed to ✅ (2 consecutive clean days, independently confirmed); a likely 10-round-old mystery may finally be solved.** Real fixes: a `medicare_ingestion` self-lock (its own parent's tracker row blocked it, not the `start_run()` header defect — my round-1 framing corrected), `investment_targets`'s CB3 upsert defect, and two schema mismatches dating to PR #7370. Not bugs after all: the staged-intake queue I flagged in round 1 turned out to be an unrelated email-intake queue (corrected), and `clinic_financial_estimates`'s zero writes were the compare-before-write fix working correctly. **Possibly the biggest find**: `hcris_cost_reports` — the original `HCRIS-TIMEOUT` target, ten-plus rounds of exception-handling fixes — turns out to 404 on its own CMS source file. See `PLANNED-BACKLOG.md`'s `HCRIS-TIMEOUT` row. PR #7428, Scott reports merged; no live run yet (next scheduled 06:00 UTC 09-25). **👤 2026-09-25: PR #7428 confirmed live across two more runs — self-lock fix holds, one open question found.** `medicare_ingestion` got its own nested tracker row and ran 1h44m to a clean `success` close, both in Scott's manual 09-24 18:30 trigger and the scheduled 09-25 06:05 run. Zero errors either run; both still closed `partial` (expected, matches the 3 still-open items from round 2). New open question: `ratings`/`clinic_quality_metrics`/`qip_scores`/`facility_deficiencies` haven't moved since the 09-24 14:xx run — possibly benign (no new CMS data, matching `facility_patient_counts`'s own no-op result both times) or possibly a new starvation shift now that `medicare_ingestion` takes real time instead of skipping instantly. Filed as `CMS-PIPELINE-STAGE-STARVATION-3`. See the dated entry below. **👤 2026-09-25, round 3 answered: it was a real bug, not benign.** `medicare_ingestion`'s retry loop was eating the run budget (both runs closed `partial`, stuck on `medicare_ingestion`, before reaching `ratings`/CQM/qip/deficiencies) — independently confirmed live. Fixed, along with the real HCRIS URLs (`facility_cost_reports`'s true source, confirmed 94,473 rows frozen since 03-16) and the `cms_deficiencies` dataset mismatch (confirmed: 0 of 463,058 rows match a dialysis clinic — it's a nursing-home dataset, now disabled by default). `census_demographics` still needs Scott: it fails every run for lack of a `CENSUS_API_KEY` on Railway. PR #7429 open, **not deployed, no live run yet**. See the dated entry below and `PLANNED-BACKLOG.md`'s `CMS-PIPELINE-STAGE-STARVATION`, `HCRIS-TIMEOUT`, and `RATINGS-CQM-CB3-upsert-class` rows. **👤 2026-09-26: PR #7429 confirmed deployed and live — real progress, not a clean close.** `ratings`/`clinic_quality_metrics`/`qip_scores`/`facility_cost_reports` all confirmed writing fresh again in the first post-deploy run — the core starvation is broken. But the run still closed `partial`, now stuck at a brand-new step (`ownership_linkage`, never seen before in this arc), and `census_demographics` still can't be confirmed working despite the API key being set. Filed as `CMS-PIPELINE-STAGE-STARVATION-4`. See the dated entry below. **👤 2026-09-26, round 4 answered: my own round-4 prompt's premise was wrong.** `ownership_linkage` was never a bottleneck — I'd only read `notes.current_step` and missed the `error_log` column, which named the real failed steps: `financial_estimates` (a 900s CPU-bound timeout, now fixed) and `census_demographics` (its key is being rejected by Census as invalid, not missing — needs Scott to check the activation email or re-paste it). A third `Prefer: return=minimal` id-loss instance (`public_data_snapshots`) and a long-standing `bd_flags` total failure (zero BD alerts ever stored, now fixed) were also found. PR #7430 open, not deployed. See the dated entry below. **👤 2026-09-26, later same day: Scott reports both PRs merged (docs #2688, and Dialysis #7430) — but no live run has tested #7430 yet.** Independently checked Supabase: the most recent `cms_ingestion` run is still the same one from before the merge (started 06:05, closed `partial` at 07:26, `error_log` still naming `financial_estimates`/`census_demographics`), `clinic_financial_estimates` is still frozen at 2026-09-24 13:06:29, and `alerts_unified` still has zero `change_source='bd_flags'` rows. That's expected, not a problem — the pipeline's next scheduled run is ~06:00 UTC tomorrow (09-27), and merging to `main` doesn't itself redeploy Railway per this repo's own standing rule. Waiting on that next run (after Railway redeploys) for live proof, and still waiting on Scott to confirm the Census key action (activation email / re-paste). |
...
## 2026-09-26 — CMS-PIPELINE-STAGE-STARVATION: both PRs merged, but no live run yet — watching for tomorrow's scheduled run

**Scott reports the docs PR (#2688) and Dialysis PR #7430 are both merged.** Independently checked Supabase before writing anything down, per standing discipline — merged is not the same as deployed or live-tested, and none of round 4's fixes have evidence yet either way:

- The most recent `cms_ingestion` tracker row for `cms_medicare_clinics` is unchanged from round 4's write-up: started 2026-09-26 06:05:00, closed `partial` at 07:26:50, `error_log` still reading `"Failed steps: financial_estimates, census_demographics; safe_execute errors: 4 total"`. No newer run exists.
- `clinic_financial_estimates` is still frozen at its last row from 2026-09-24 13:06:29 — no new data since the memoization fix merged.
- `alerts_unified` still has zero rows with `change_source='bd_flags'` — no new BD flag has been written since the fix merged (none would be expected without a new run).
- The `census_demographics` row in `public_data_snapshots` is still the same 07:10:08 `started`/never-closed row from before the merge.

None of this means the fixes don't work — it means the pipeline hasn't run again since the merge landed. It's currently 2026-09-26 14:02 UTC; today's scheduled 06:00 UTC run already happened (and is the pre-merge run above), so the next scheduled run is ~06:00 UTC tomorrow (09-27). Also flagging, per this repo's own standing instruction, that a merge to `main` is not itself a Railway deploy — both `tranquil-delight` and the standalone MCP service need their own redeploy for engine-code changes to actually take effect, and that hasn't been independently confirmed to have happened yet either.

**Still open, unchanged from round 4:**
- `census_demographics` still needs Scott's action — check the Census signup email for an activation link, or re-paste `CENSUS_API_KEY` on Railway.
- No live run has tested `financial_estimates`'s memoization fix, the third `public_data_snapshots` id-loss fix, or the `bd_flags` fix.

**Next step:** watch for tomorrow's scheduled run (or a manual trigger, if Scott runs one sooner) and independently re-verify all four fixes against Supabase before calling any of them confirmed.

---
## 2026-09-26 — Round 81 (Cowork): DUP-RECORDS1 reconciled live; sidebar-sale feed cron applied; Scott's lane errors explained; Available accuracy measured → `AVAIL-TRUTH1`

**Deploy.** `verify:deploy`: `tranquil-delight` is on `7fb2135f`, which contains #2687. The route `/api/sidebar-sale-feed` is served, so Cowork applied the held cron (**269**, 05:40 UTC). A smoke run returned 200 and staged dia 288 / gov 200 rows for the 05:50 promoter.

**Scott worked the Accuracy lanes (Q62).**
- 5 listing-sale cards applied cleanly: 4 `confirm_sold`, 1 `reject`.
- 3 clicks errored `review_not_open` (Kissimmee 37696 / 24669, Oak Forest 25570). DUP-RECORDS1 merged those duplicate properties in the same minutes (12:30–13:24 UTC), which superseded the underlying reviews. The cards were stale, not wrong.
- Cowork superseded the 3 cards and filed `REVIEW-LANES1-stale-cards`, so a settled card says so instead of erroring.
- A separate `match_disambiguation` pick fails a CHECK (23514) → `DECISION-MATCH-DISAMBIG-CHECK`.
- **Open for Scott:** 15 listing-sale cards (dia 6, gov 9).
- **Not for hand-working:** asset relinks 14, owner reviews ~646, contact conflicts 55, and the legacy dia twin lane (1,237 pending).

**DUP-RECORDS1, live (CC):**
- dia two-signal merger with CCN / operator / address / directional vetoes;
- Kissimmee → 24669 and Oak Forest → 25570 merged; Marathon listing repointed to 41088 and closed;
- 10 more dia pairs merged and 71 carded;
- 84 gov owner duplicates merged, with the hub merge-follow repointing 24 contacts;
- a junk-address guard on both property tables;
- Dayton's dia double listing folded onto 27901 (the 1403/1431 building question stays a card: no parcel on either row).
- Filed follow-ups: `-gov-properties`, `-panel-merge`, `-hub-conflicts`, `-dia-junk-rows`, `-vetoed-pairs`.

**Scott's Available-accuracy question, measured live.**
- **Sold:** handled. The parity check reads 0.
- **"No longer on the market": nothing checks.** Gov has 463 active, dia 425. Neither DB has ever URL-checked a listing (gov 458, dia 425 unchecked). `lcc_listing_web_pages` has **0 rows**, so the 30-minute crawler has nothing to crawl. On gov, 355 listings haven't been seen in 90 days; 357 are OM snapshots.
- **Duplicates:** 8 gov pairs are the same building listed on twin property ids, e.g. Tupelo $5.64M vs $5.135M. A re-capture at a new price made a second listing instead of recording a price change.
- **Price/date history:** gov `price_change_count > 0` on 8; dia `price_change_history` is empty on every row; no per-event log exists; gov `on_market_date` is null on 233.
- Everything above is prompted as `AVAIL-TRUTH1`:
  - a gov property merger, plus the panel-merge fix;
  - one listing per offering, with a price/status event log;
  - re-capture updates the existing listing instead of inserting a new one;
  - liveness: register URLs for the existing crawler, sync status from SF, and move OM-only listings to "unverified" after N days;
  - a "truly available" headline count;
  - guards.

**Consolidation.**
- Five final rows archived verbatim, with CURRENT-STATE §2b pointers.
- Q61 archived; Q62 rewritten to what's actually left.
- The DUP-RECORDS1 prompt moved to done/.


## 2026-09-26 — CMS-PIPELINE-STAGE-STARVATION-4 answered (CC): correcting my own wrong premise; real fixes for financial_estimates and a long-standing bd_flags failure; census key needs Scott

**Correction, not silently fixed: my own round-4 prompt was built on a wrong read of the data.** Earlier today I filed round 4 believing `ownership_linkage` was a new starvation bottleneck, based on `ingestion_tracker.notes.current_step` showing it as the run's last recorded step. I missed the `error_log` column entirely, which independently confirms the real answer: `"Failed steps: financial_estimates, census_demographics; safe_execute errors: 4 total"`. `ownership_linkage` is simply the last step in the pipeline's step list — it's always what shows up when a run finishes normally, not evidence of anything wrong. Noting this here rather than quietly moving past it, per this session's own standing rule about correcting itself out loud.

**The real bug, found and fixed: `financial_estimates` timed out at 900s from a CPU-bound loop, not I/O.** It read all ~190k patient-count rows in 29 seconds, then made zero database requests for 14.5 minutes before the timeout hit. `clean_fields_for_table` takes ~5ms per row, which adds up to ~16 minutes across 190k rows — more than the 900s budget. Since the financial estimate depends only on the patient count, it's now computed once per distinct count (a few hundred) instead of once per row. Independently confirmed: `clinic_financial_estimates` hasn't gotten a new row since 2026-09-24 13:06:29 — consistent with this step failing on both the 09-25 and 09-26 runs.

**`census_demographics` did run — confirmed independently** (a `public_data_snapshots` row for it started at 07:10:08, before `ownership_linkage` even ran) **but Census is rejecting the key as "Invalid Key."** That's a different failure than "missing" — Railway is sending a key, Census just doesn't accept it. Most likely the activation link in Census's signup email was never clicked, or the key was copy-pasted with stray characters (the code now strips those automatically, and the error message now reports the key's length and last 4 characters so it can be compared against the signup email). **Action needed from Scott: check the Census signup email for an activation link, or re-paste the key.**

**A third instance of the `Prefer: return=minimal` id-loss defect (same class as round 1's `start_run()`) was found and fixed.** `public_data_snapshots` rows never close — independently confirmed 5 of the last 10 `census_demographics` rows there show `run_status='started'`, `finished_at=null`, dating back to 2026-04-08.

**`bd_flags` turned out to be worse than believed, and is now genuinely fixed.** `bd_flags` is a *view* over `alerts_unified` that doesn't expose `entity_type` — independently confirmed (its column list has no `entity_type`; `alerts_unified`'s does, and it's NOT NULL) — so every insert through this view has always failed. Independently confirmed the scope: `alerts_unified` has **zero** rows with `change_source='bd_flags'`, ever. No BD flag — including CMBS-distress signals — has ever actually been stored. The failure also tripped the circuit breaker, silently dropping the rest of that run's flags too, and hid a duplicate check that always errored. Fixed: `create_bd_flag` now writes `alerts_unified` directly with a working duplicate check; `log_alert_unified` now reports failures instead of masking them as success; `resolve_bd_flag` normalizes flag types so they can actually match a stored flag.

`recorded_owners` is unchanged — still the one known duplicate-key row (`V V Continental Llc`), doesn't block anything new.

**Status: pushed to `claude/gracious-pasteur-dsysue`, PR #7430 opened per CC** (this session has no direct GitHub access to the Dialysis repo, so the PR's existence and CI state aren't independently verifiable from here). CI had not reported at write-up time. **Not deployed, not live-tested.**
...
**Confirmed fixed, real writes:**
- `ratings` (`updated_at` 06:49:53), `clinic_quality_metrics` (06:40:55), and `qip_scores` (07:11:57) all moved for the first time since the 09-24 14:xx run — direct proof the `medicare_ingestion` retry-loop fix broke the starvation that was blocking them.
- `facility_cost_reports` grew from 94,473 to 101,619 rows (+7,146) — confirms the corrected HCRIS URLs are pulling real data.
- `facility_deficiencies` correctly stayed frozen at 463,058 rows — expected, since it's disabled by design now, not a bug.
- `cms_medicare_clinics`'s nested run dropped from 1h44m to 31 minutes, matching the fix's estimate.

**Not fully closed — the starvation shifted, it didn't disappear:**
- The run still closed `run_status='partial'` (81.8 minutes, an improvement over 108 but still not a clean finish), with `notes.current_step = "ownership_linkage"` at close. Independently confirmed this is the **first time this exact step has ever been the run's closing step** in this table's history — previously it was always `medicare_ingestion` or unrecorded. This is the same pattern the whole arc keeps finding, just relocated again.
- **`census_demographics` can't be confirmed working**, despite the API key being set: no census-named tracker row appeared in this run, no error was logged for it, and both candidate census tables (`property_demographics`, `census_zcta_demographics`) remain frozen from March/April. The run most likely never reached that step before closing at `ownership_linkage`. The key being set doesn't by itself prove the code path ran.

**Also found, not part of this arc:**
- A new `bd_flags` insert failure: `alerts_unified.entity_type` violates a NOT NULL constraint, cascading into `circuit_open` on subsequent `bd_flags` inserts.
- `recorded_owners`'s previously logged-not-fixed CB3 duplicate-key defect recurred (`V V Continental Llc`).
- The `ingestion_run_errors` logging gap from round 3 appears resolved — 4 real errors logged this run, the first since 2026-09-24 15:04:45.

Filed as `CMS-PIPELINE-STAGE-STARVATION-4` to find and fix whatever now gates `ownership_linkage`, confirm whether `census_demographics` is actually reachable, and take the two new/recurring defects.

---

## 2026-09-25 — CMS-PIPELINE-STAGE-STARVATION-3 answered (CC): the staleness was a real bug, not benign; HCRIS URLs found; two of three blockers resolved

**Question 1 — answered: it's a real starvation bug, not benign.** Both the 09-24 18:30 and 09-25 06:05 runs closed `run_status='partial'` with `notes.current_step` stuck at `medicare_ingestion` — independently confirmed live against `ingestion_tracker.notes` on both parent rows, not taken on CC's word. The mechanism: `medicare_ingestion`'s `retry_missing_records()` checks "does this clinic already exist" with an unpaged read that PostgREST caps at 1,000 rows, so ~7,500 of 8,547 real clinics looked missing on every run; every one of the ~14,000 resulting re-saves came back HTTP 400 and never landed. Independently confirmed: `clinic_history_unified` gained exactly 10,523 junk address-change rows since 09-24 18:00 (not yet deleted, flagged for cleanup). The step's own 45-minute timeout fired around 06:53 both times but was caught as a routine failure and kept looping; the run's 90-minute budget cap then stopped the whole run at 108 minutes, before `aux_cms_tables` — so `ratings`/`clinic_quality_metrics`/`qip_scores`/`facility_deficiencies` were genuinely never reached. Fix: the read now pages through all rows, the retry pass skips itself rather than re-running against every clinic if it can't finish, and the step timeout now actually ends the step (should bring `medicare_ingestion` down to roughly 17 minutes).

**A related logging gap, found and flagged (not root-caused):** `ingestion_run_errors` has logged zero rows since 2026-09-24 15:04:45 — independently confirmed exact match — even though ~14,000 saves were rejected in these two runs. The helper that made those saves swallows the error before it reaches the ledger. CC didn't find why; it's deprioritized since the retry-pass fix should stop touching existing clinics at all, but worth remembering if a future round sees "zero errors" and assumes a clean run again.

**Question 2 — two of three blockers resolved, one needs Scott:**

- **HCRIS cost reports — fixed.** All four URLs previously guessed in the code were wrong (all 404). CC read CMS's own cost-reports page through the database's own outbound web access (the coding sandbox can't reach `cms.gov` directly — a real constraint, not an excuse) and found the real files: `RNL11-ALL-YEARS.zip` and `RNL11-REPORTS.zip`, both confirmed to return real ZIPs. `facility_cost_reports` (this arc's actual table — "hcris_cost_reports" has been this session's shorthand for it) independently confirmed to hold 94,473 rows, frozen at 2026-03-16, matching CC's figure exactly. Ten-plus rounds of exception-handling work finally has a real URL to try.
- **cms_deficiencies — resolved as "wrong dataset," not a timeout to fix.** The CMS dataset behind this table (`r5ix-sfxw`) is **nursing-home** deficiencies, not dialysis. Independently confirmed: **0 of 463,058 rows** match a dialysis clinic's `medicare_id`, and the table's row count/last-updated (463,058 / 2026-09-24) match CC's figures exactly. Turned off by default (`CMS_ENABLE_NURSING_HOME_DEFICIENCIES=1` to re-enable); `facility_deficiencies` will stay stale by design because CMS doesn't publish a dialysis-specific deficiency dataset here.
- **census_demographics — needs Scott.** Without an API key, `api.census.gov` returns an HTML "Missing Key" page with a 200 status, which broke the JSON parser. `CENSUS_API_KEY` isn't set anywhere. **Action needed: get a free key at [api.census.gov/data/key_signup.html](https://api.census.gov/data/key_signup.html) and set `CENSUS_API_KEY` on the Railway cms-ingestion service.** Its upsert also had the same header-defect (CB3) class bug as `ratings`/CQM — fixed as part of this round (another instance, 9 of ~133 now fixed by name).

**The recorded/watermark tracker rows are explained, not new.** `watermark` is the data-version stamp written when the medicare ingest completes; `recorded` is a redundant second stamp with no closing code path, dating back to 2026-03-24. All 27 such rows (including `8622dc69…` and `8f615de8…`, the ones flagged last round) were still open — independently confirmed — and have now been closed in the database (`finished_at = started_at`, reversible); new ones are written already closed.

**Status: not deployed.** Fix pushed to `claude/beautiful-galileo-kme2el`, opened as PR #7429. CI reported 3,405 passed / 2 failed at write-up (both pre-existing on unmodified `main`). No live run has tested any of round 3's fixes yet — the next run after deploy is what actually answers whether `ratings`/`clinic_quality_metrics`/`qip_scores` move once `medicare_ingestion` stops eating the run budget.

---
## 2026-09-25 — Round 80 (Cowork): SALE-PROMOTER1 + REVIEW-LANES1 reconciled live; lanes tick cron applied; Dayton decided (gov out, dia double listing found); `DUP-RECORDS1` prompted

**Deploy.** `verify:deploy`: `tranquil-delight` is on `4bc7d803`, which includes #2682 and #2683.
- REVIEW-LANES1 said to apply its tick cron only after the deploy. The route is served, so Cowork applied `20261102370000_lcc_review_lanes1_tick_cron.sql` (**cron 268**, 06:17 and 18:17 UTC).
- A smoke tick returned **200**. Open lanes: listing-sale 24, asset relinks 14, owner reviews 646 (2 more auto-resolved), contact conflicts 53.
- The listing-sale lane grew from 18 to 24 because SALE-PROMOTER1 queued its own reviews. Its auto-resolver deliberately leaves those for a human.

**SALE-PROMOTER1: verified live.**
- The four gov listings (Walla Walla 23342, Savannah 16265, Moses Lake 16412, Jasper 16236) are `sold` and linked.
- Cron `gov-sale-promoter` runs at 05:50. `gov_check_listing_sale_parity()` reads 0 active listings with a sale and 10 open reviews. Gov active listings: 464.
- The remaining named cases (Bronx, Durham, Asbury Park, Marathon, Oak Forest, Kissimmee) are in review. Most sit on duplicate property rows.

**Scott's calls.**
- **Madison WV:** correct as is.
- **Dayton:** "if it doesn't fit in either database, remove it."
  - **Gov:** listing `15f861cb…` on 41071 is now `off_market` / `out_of_domain_no_gov_or_dia_tenant`, logged in batch `cowork_r80_dayton_out_of_domain_20260925` (restorable).
  - **Dia:** Cowork checked before removing anything, and it **does** fit. The building houses **DaVita Wright Field Dialysis** (property 27901, with a Medicare clinic). It is also listed **twice**: listing 15006 on property 38412 ($2,037,500) and 15281 on 27901, same broker and 7.40% cap. That's a real dia double count, handed to `DUP-RECORDS1` rather than guessed. The two properties differ on SF, so they may be separate buildings.

**Consolidation.**
- Three final rows archived verbatim, with CURRENT-STATE §2b pointers: CONTACTS-GOV-WRITER, LISTING-SALE-PARITY1, -stale-volume.
- Q61 answered; Q62 added (work the listing-sale lane once).
- The REVIEW-LANES1 prompt moved to done/. SALE-PROMOTER1's prompt had already been moved by CC.
- The other session's `CMS-PIPELINE-STAGE-STARVATION-3` prompt was left untouched.

**Next.** CC: `DUP-RECORDS1`. Duplicate property and owner rows are now the common blocker behind SALE-PROMOTER1's refusals, the review lanes and the Dayton double listing. It applies Scott's Saginaw rule (merge into the most accurate record; reversible; merge only on two signals) and stops duplicates at the writer.
...
- **`medicare_ingestion`'s self-lock fix holds.** It now opens its own nested tracker row (e.g. `b8c4ed09…`, started 06:08:48, ran 1h44m41s, closed `success`) instead of reading its own parent's row as a competing run. `medicare_clinics.source_last_seen` moved again (07:53:17 UTC). This is the second run in a row proving the fix, not a one-off.
- **`facility_patient_counts`'s self-reporting instrumentation works as designed both times**: `"processed_no_writes (all unchanged / no new CMS period)"`, 7,490 processed, 0 inserted/updated, 7,468 unchanged.
- **Zero errors logged in `ingestion_run_errors` in either run.** Both still closed `run_status='partial'` — expected, matches the three items round 2 left open and unfixed (`hcris_cost_reports`'s 404, `cms_deficiencies`'s step timeout, `census_demographics`'s non-JSON response).

**Open question, not yet resolved either way**: `ratings`, `clinic_quality_metrics`, `qip_scores`, and `facility_deficiencies` haven't moved since the 09-24 14:08-14:47 UTC run — neither of these two later runs touched any of them. Two competing explanations, and I can't distinguish them from Supabase data alone:

1. **Benign**: CMS genuinely hasn't published a new snapshot since 09-24, so every step correctly finds nothing new — consistent with `facility_patient_counts`'s own "no new CMS period" result holding in both later runs.
2. **A new starvation shift**: now that `medicare_ingestion` actually runs for real (1h45m) instead of skipping itself instantly, it may be consuming enough of the run's step budget to prevent later steps from being reached — the same shape this whole arc keeps finding, just relocated now that the previously-broken step works.

**Also new and unexplained**: a tracker row with `run_status='recorded'` (`8f615de8…`, started 06:25:16 UTC) has sat open with `finished_at=null` for 5+ hours as of this check, and a `run_status='watermark'` row closed instantly at the same timestamp — neither state has appeared anywhere else in this saga. Likely new instrumentation from round 2's fix that wasn't documented in CC's write-up, but flagged rather than assumed benign.

Filed as `CMS-PIPELINE-STAGE-STARVATION-3` (prompt: `CMS-PIPELINE-STAGE-STARVATION-3-order-of-ops-and-remaining-blockers.md`), bundling this open question with the three still-unfixed items from round 2, the two logged-not-fixed CB3 instances (`recorded_owners`, `bd_flags`), and the unexplained `recorded`/`watermark` tracker states.
...
**LISTING-SALE-PARITY1: verified live.**
- `gov_check_listing_sale_parity()`: `active_listings_with_sale` 0, `open_reviews` 7.
- Active listings: gov 469 (482 before the round), dia 437.
- Lakeland FL 31007 is `sold` and linked to its 2026-06-09 sale.
- Madison WV 15747 stays active on purpose: `on_market_date` 2026-03-24, after the 2022 sale; three March-2026 duplicates were already superseded. Put to Scott in Q61.
- A correction to our round-78 read: CC found **dia's** rules had also drifted (a historical sale could close a current re-listing; the hygiene sweep closed on non-market sales). Both DBs now call one verdict, `lcc_listing_sale_verdict`, byte-identical on both.
- Cross-domain: 4 buildings were open in both DBs.
  - Long Beach and Fairborn are DaVita with no gov tenant, so CC pulled the gov copy (reversible).
  - Cookeville has both tenants, so it stays in both.
  - Dayton is a vacant MOB → Q61.
- The new test files need a local Postgres, so they skip in this VM; CI ran them green.

**SEC7 phase 2a: verified live.** `dia_/gov_/lcc_sec7_write_path_violations()` and both ledger checks read 0.
- The 7-day writer inventory found no anon or authenticated writes. The only anon writer anywhere is Power Automate's `lcc_record_flow_failure` RPC, left as is.
- The views keep SELECT.
- Phase 2 (dia 58 / gov 48 / LCC 124 RLS-off tables) is unchanged and stays filed, Supabase branch first.

**Also verified.**
- **CONTACTS-GOV-WRITER at +24 h:** 0 writes, guard total 0. The last gov copy write is still the cutover run.
- **MERGELOG guard:** both crons ran their first tick at 06:50 and 06:55 UTC and succeeded.

**Next prompts.**
- `SALE-PROMOTER1`: a guarded writer for the 11 sales Salesforce or CoStar already know, so their listings close through the new triggers. It also re-keys the stale rule on capture date.
- `REVIEW-LANES1`: four queues filled by recent accuracy rounds have no consumer (listing-sale 15, asset-link 14, contacts 728 + 53 conflicts). They become Decision Center lanes, with auto-resolve first.
- The two touch different code, so they can run in parallel.

**Consolidation.**
- Three final SEC7 rows archived verbatim, with CURRENT-STATE §2b pointers.
- Both returned prompts moved to done/.
- Q61 added.


## 2026-09-25 — LISTING-SALE-PARITY1 (CC): sold properties off Available, and dia and gov now close listings the same way

**Cause.** Both gov sale→listing triggers matched `'Active'` against a lowercase vocabulary, so they
never fired. Nothing on either DB closed a listing captured after its sale. Dia's four closers each
applied a different rule.

**Shipped, live on both DBs.**
- One pure verdict, `lcc_listing_sale_verdict`, byte-identical on both (live md5 `665d1e5f…`),
  with one reversible writer per DB.
- Sale, sale-event and listing-side triggers all call it. Ambiguous cases go to review queues.
- A daily read-only guard raises `listing_sold_still_active`, and stale listings go to verification.
- Gov: 9 closed (incl. Lakeland/Pipkin), 7 queued. Dia: 2 closed, 8 queued.
- Gov active 482 → 471, `v_gov_on_market` 333 → 322. Dia active 441 → 439. Guard 0 on both.
- 4 cross-domain doubles found: Long Beach and Fairborn gov copies quarantined (dia owns).
- 11 listings are sold per SF or sidebar with no sale row. No guarded writer exists for those, so
  they're listed only (`-sf-promoter`).
- Also fixed: the gov sweep's `'Active'` ranking, and `gov_listing_propagate_to_sale` aborting the
  write on a 0.0007 cap.

**Deploy.** DB only; no LCC JS changed, so no Railway redeploy. Writeup:
...
**Deploy.** `tranquil-delight` is on `8f0c784c` (#2675 docs, #2676 SEC7-LEDGERS; neither changed LCC runtime code). Per CC's read-back, SF edge `intake-salesforce` is v38 and `intake-salesforce-files` v35, so NCUA/FCA route to gov.

**Verified live.**
- **Agency registry:**
  - 0 gov properties disagree with `gov_resolve_agency` (not counting manual pins);
  - gov cron 53 is scheduled;
  - 33519 now resolves through the new trigger to `TX-HHSC`.
- **SEC7 phase 1:** `dia_sec7_ledger_privilege_violations()` and `gov_sec7_ledger_privilege_violations()` both return 0 rows.
- **Contacts guard at +7 h:** 0 writes.

**Scott's Gov Available observation, measured.**
- **The main cause: the gov sale→listing close never fires.** Gov stores `listing_status` in lowercase (`active` 482 / `sold` 2,308 / …). Both gov triggers (`close_listing_on_sale`, `pse_close_listing_on_sale`) test `= 'Active'`. They also close only one listing, require `listing_date <= sale_date`, and never link `sale_transaction_id`; none of the 482 active gov listings is linked.
- **Dia's trigger is correct.** It closes all open listings, skips non-market sales, keeps a re-listing that comes 90+ days after the sale, and links the sale.
- **Machinery dia has and gov lacks:** listing consolidation by transaction and property, sale-event mark-sold, closed-sales→workbook, and the daily CM listing snapshot.
- **Neither DB catches a listing created after the sale is already on file.** Listing dates are often capture fallbacks. Examples:
  - Lakeland FL 31007: sold 2026-06-09, OM captured today;
  - Madison WV 15747: sold 2022-02, still active;
  - also 11 twin-address sales, and gov 14 / dia 13 stale listings.
- **Cross-domain double count:** unmeasured.

Everything goes to CC as one parity prompt: a matrix of both DBs, gov fixed to dia's semantics, a reverse check on both, a logged backfill, a double-count measure, "should be sold" from SF/CoStar, and a daily guard.

**SEC7 phase 2a.** CC proved anon can rewrite all 12,697 gov `ownership_history` rows and all 5,009 dia `sales_transactions` rows through writable definer views, and silence CMS ingestion through a `WITH CHECK (true)` policy. CC asked Scott whether to close the view route. Cowork recommends yes, since reads are untouched, and drafted `SEC7-PHASE2A` with a writer inventory first.

**Consolidation.**
- Six final rows archived verbatim, with CURRENT-STATE §2b pointers: GOV-REGISTRY2, -promoter-schedule, -agency-id-stale, MERGELOG-GAP, CONSOLIDATE-REVERSIBLE-reconcile-unmerge, DIA-REDIRECTS-ANON-WRITE.
- Both returned prompts moved to done/.

**Next (CC).** `LISTING-SALE-PARITY1` first (Scott's accuracy issue). `SEC7-PHASE2A` can run in parallel because it touches grants and policies, not listing logic.
...
- **Gap 3 — `medicare_ingestion` self-lock.** The pipeline opens its own `cms_medicare_clinics` tracker row, then the medicare step asks for a lock on that same dataset and finds its own parent's row, reading it as "another run active." **This corrects my own round-1 documentation**, which called this "the same open-tracker-row lock" defect as `start_run()`'s missing-id bug — it's a different mechanism (lock-scoping, not a lost id) that PR #7427 couldn't have fixed. The lock now ignores rows its own process opened; a lock-denied skip now fails the step instead of counting as "ok."
- **`investment_targets`** fixed with the same `postgrest_prefer.py` helper as the other six tables.
- **Two schema mismatches** dating to PR #7370 (2026-08-13): `hcris_propagation` passed a field (`payer_mix_commercial`) a dataclass never declared; `property_financials` selected a column (`medicare_clinics.total_patients`) that doesn't exist. Both only surfaced now because this is the first run that ever got far enough to hit them. Both fixed.
- **Why every error this run landed on the wrong tracker row** (a new finding): `start_run()` points the process-wide error log at whichever row it just opened, clearing prior entries. The medicare self-lock's nested lock row hijacked error recording for the rest of the run — that's why `bd_flags`/`recorded_owners` errors appeared filed under the `facility_patient_counts` lock row. Fixed; historical error rows still carry the wrong id.
- **The tracker `notes` field** wasn't cross-contaminated (Gap 5) — `finish_run()` was stamping a *global* intake-queue count onto whichever row it closed, and wiping the step heartbeat at close. Fixed: scoped to the pipeline's own row under a clearer key, heartbeat preserved.

**Not bugs — this session's own reads corrected, not silently:**

- **Gap 1**: the "994 queued / 2 promoted" counts I reported as `facility_patient_counts`'s own staging queue are `staged_intake_items`, the unrelated email/document-intake queue (rows from April 14-21). The real question — why `patient_counts` derived 0 rows — can't be answered from Supabase alone (CC's sandbox has no CMS egress); the step now records its own outcome so the next run self-reports the answer.
- **Gap 2**: `clinic_financial_estimates`'s zero writes were correct — CC ran the repo's own calculation against 1,500 live rows and all matched, because the prior run had just finished rewriting everything hours earlier. Compare-before-write is working as designed. Side finding, not fixed: 5 columns outside the writer's schema (payer-mix/revenue share fields) silently never get written; fixing it means a ~190k-row rewrite, Scott's call.

**Possibly the biggest find of this round**: `hcris_cost_reports` — the table this arc has chased for ten-plus rounds via `HCRIS-TIMEOUT` — **gets a 404 fetching CMS's own `RENAL_COST_REPORT.zip`.** Every prior round's fix (timeout propagation, `safe_execute()` layering, `ThreadPoolExecutor` shutdown, compare-before-write, starvation) was real and necessary, but none of it was ever the reason this specific table stayed frozen since 2026-03-16 — the pipeline simply never got far enough to discover the source file itself was unreachable, until this round's starvation fix let it run that far. Not yet fixed — needs its own round to confirm whether the URL moved and find the replacement. See `PLANNED-BACKLOG.md`'s `HCRIS-TIMEOUT` row (now 🟡 after being stuck at 🔴 for ten rounds).

**`RATINGS-CQM-CIRCUIT-BREAKER` closes further**: independently queried `ingestion_run_errors` by day rather than trusting per-run reads — `ratings` logged zero errors on both 2026-09-23 and 2026-09-24, meeting the two-consecutive-clean-day bar this item set for itself. **Closes to ✅.** `clinic_quality_metrics` has one clean day of the two required (2026-09-23 still had 955 errors from before PR #7426 took effect), stays 🟡.

**`RATINGS-CQM-CB3-upsert-class`**: two more instances confirmed live this round (`investment_targets`, fixed by name; `recorded_owners`, a create that never links because it reads an empty `return=minimal` body — logged, not fixed). That's 8 of ~133 flagged call sites hit by name, one discovery at a time; CC again recommended a dedicated class-wide round with its own census rather than folding it in opportunistically.

**Tests**: 28 targeted (including reproductions of the self-lock and upsert bug pre-fix), full suite 3,389 passed / 2 pre-existing unrelated failures. **Delivery**: branch `claude/modest-feynman-f61w5h`, PR `sbriggssjc/Dialysis#7428` — Scott reports merged. **Independently confirmed no live run has tested this fix yet** — no `ingestion_tracker` row has started since the prior run finished at 15:08 UTC; next scheduled run is 06:00 UTC 2026-09-25. Watch then for `medicare_clinics.source_last_seen` moving past 2026-08-31, `facility_patient_counts`'s lock row carrying a real `patient_counts_outcome`, and no further `investment_targets` `circuit_open` errors.

**Repo cleanup this round**: the CoStar/PRI open-threads row (this table, above) had grown to 12,541 characters against its own "one-line read" rule — the entire `HCRIS-TIMEOUT` rounds 1-10 narrative (2026-09-16 through 09-22) archived verbatim to [`docs/history/STATUS_open-threads_PR5-PRI-row_to_2026-09-22.md`](../history/STATUS_open-threads_PR5-PRI-row_to_2026-09-22.md), same convention already used for the SBN row. Nothing was dropped or reworded — the still-active `RATINGS-CQM-CIRCUIT-BREAKER`/`CMS-PIPELINE-STAGE-STARVATION` narrative stays live in the table.
