...
## 2026-09-12 — CONSOLIDATE3 items 1+3 shipped: headroom crisis fixed, table-placement guard added (Cowork)

STATUS.md was at 2,498 of a 2,500-line budget — one entry from anyone failing the suite. Also found
and fixed a real ordering bug: an earlier merge landed a new entry ABOVE the Open-threads table
(two independent same-day PRs both touching line 1, git merged them in file order rather than
intent order). Fixed: table restored to the top, budget raised to 3,000 with an 80%-mark soft
warning (`console.warn`, doesn't fail the suite) and a documented 60%-of-budget archive target, plus
