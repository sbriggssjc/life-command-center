...
| PL-57 | 2026-09-18 | → STATUS round 44 (process) — *CC titled its HOME2-d entry "Round 43 (Cowork)" — a Cowork label on a CC round; headings are append-only so it stays; Cowork is 44. Prompt header rule tightened again (PL-55) (Cowork)…* |
| PL-58 | 2026-09-18 | → backlog `RECON2-d-reconcile` — *CC's RECON2-d response says "none of the RECON2/RECON2-b/RECON2-c source files this prompt referenced actually existed in the repo" — true only because it was in the Dialysis repo; the files are in `life-command-center/supabase/migrations/dialysis/`. A prompt must name the repo root in its first line (Cowork)…* |
| PL-59 | 2026-09-18 | → backlog `SIDEBAR3` — *there is no sidebar capture ledger; `properties.last_ingested_at` is the only per-send trace and it holds no payload. A `sidebar_captures` table (payload hash, property resolved, rows written) is the precondition for every SIDEBAR measurement (Cowork)…* |
| PL-60 | 2026-09-18 | → backlog `DIA-PROPAGATOR1` — *Supabase edge logs are queryable per UA/IP/path via the MCP `query_logs` tool (ClickHouse SQL); this is how the runaway writer was found in one query — add to the reconcile README as step 4b: "who wrote to this DB today?" (Cowork)…* |
| PL-61 | 2026-09-18 | → backlog `HOME2-e` — *a CSS rule-presence test is not a layout test; the HOME2 lane tests should compute specificity or the round must carry a Railway computed-style probe (Cowork)…* |
| PL-62 | 2026-09-18 | → backlog `PERF-SPQ2` — *cold Home load on `0abc89f7`: one request ran ~20 s while the BD + INBOX lanes spun; `work_counts` answered in 0.8 s — the seller-prospect queue is still the cold-start long pole even after PERF-SPQ1-c (Cowork)…* |
| PL-63 | 2026-09-18 | → backlog `DIA-PROPAGATOR1` — *the propagator's cumulative PATCH families include `true_owner_id` on `properties`; if it still writes that column it is a second, unguarded owner writer beside ORE/OWNERGAP — the Dialysis prompt must inventory every column it writes and name the owner of each (Cowork)…* |
