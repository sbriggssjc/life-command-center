...
|---|---|---|---|---|
| Q1 | **A** | **Re-check the calendar flow.** After your four header edits, `POST /sync/calendar-events` still logged `DENY-WOULD` at 18:26 UTC (hourly, unchanged). Open the flow whose HTTP action targets `…/ai-copilot/sync/calendar-events` (workflow id `4eb7c46f…` in `docs/architecture/flows/ai-copilot-sync-callers.md`; it may not be the one you edited) and confirm the header is exactly `X-PA-Webhook-Secret` with the Object Sync flow's value, saved. The other three (`/sync/activities`, `/sync/sf-tasks`, `/sync/flagged-emails`) run too rarely to judge yet — Cowork reads the log tomorrow; 3 clean days → `COPILOT_AUTH_MODE=enforce`. | the hourly caller is the one still unauthenticated | COPILOT-OPEN |
...
| Q29 | **D** | Your own name appears three times in `brokers` (ids 1373 / 2076 / 2437; one linked to firm 126, two blank) — say which is you and whether the other two are duplicates; BR4 left them alone by rule. | 30 seconds | BR4 (PL-13) |
