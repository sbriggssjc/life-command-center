...
| Q30 | **A** | **The LCC window you use is the old Vercel site, running old code.** ☐ Open `https://tranquil-delight-production-633f.up.railway.app` in Chrome, sign in, and look at Home — that is where the three-lane Home (and everything since early September) lives. Tell Cowork what Home looks like. ☐ If it works: uninstall the current LCC desktop app (⋯ menu → Uninstall) and install it again from the Railway URL. ☐ Do **not** delete the Vercel project yet — Cowork needs its env-var list compared with Railway's first (Vercel → Project → Settings → Environment Variables: names only, no values). | ☐ | VERCEL-LIVE1, HOME2 |
