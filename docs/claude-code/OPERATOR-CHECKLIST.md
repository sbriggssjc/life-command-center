...
|---|---|---|---|---|
| Q1 | **A** | ~~Re-check the calendar flow~~ **Done 2026-09-17** — the hourly caller was a second flow ("Outlook Calendar - Life Command Center Sync"); header added. Log: last `DENY-WOULD … /sync/calendar-events` 18:26 UTC, clean since. Nothing to do until the 3-day clock runs: **earliest `COPILOT_AUTH_MODE=enforce` = 2026-09-20 evening**, Cowork re-reads the log each turn and confirms the three rarer routes (`/sync/activities`, `/sync/sf-tasks`, `/sync/flagged-emails`) as they fire. One blocker surfaced: an unnamed `POST /chat node other` caller (PL-14) — if you know of anything besides the browser and Railway that calls the copilot chat route, say so. | waiting on the clock | COPILOT-OPEN |
