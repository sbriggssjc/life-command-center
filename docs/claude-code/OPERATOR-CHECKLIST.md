...
| Q45 | **B** | ☐ **Re-send the Findlay US Renal OM from Salesforce.** Its first pass was the one that put our Tulsa office on the gov Available list (GOV-AVAIL1). The file wasn't kept, and dialysis has three US Renal properties in Findlay, OH, so it can't be re-placed from here. In Salesforce, open Listing `a0jVs00000GuTfdIAF` (file `USRenalMOB_Findlay_OH_OM_SB.pdf`, ContentVersion `068Vs000019XB2IIAW`) and re-send it. Cowork then checks that it lands on the right dialysis property and never in gov. | the real Findlay listing is not on file anywhere yet | GOV-AVAIL1 |
| Q46 | **C** | ☐ **Automation decisions from GOV-UX1 §D** (measured; `docs/audits/GOV_UX1_NAVIGATION_OWNER_VERIFICATION_2026-09-22.md`). Reply with the ones to build: **D1** fix the agency-drift detector (recommended first) · **D2** GSA → occupying agency (~272) · **D3** fill blank agency from lease (~46) · **D4** auto-cadence on open leads without one (35) · **D5** gated auto-lead: only seller-queue owners with a reason to sell + linked person (98). Everything else stays human. | answers "automate as much as possible" with a measured, logged rule set | GOV-UX1-D1…D5 |
