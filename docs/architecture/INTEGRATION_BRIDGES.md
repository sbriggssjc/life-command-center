> 🚨 **SUPERSEDED/STALE (DOCMAP3, 2026-09-24; moved from `docs/INTEGRATION_BRIDGES.md`):** all Vercel / `vercel.json` / 12-function guidance is obsolete. Routing is `server.js` on Railway (`/api/enrichment-worker`, `/api/salesforce-changes`, `/api/admin/bridges`), and Phases 1 and 3 (SF bridges, `email_bodies`, `meetings`) are live. The allowlist and write-policy model below still holds.
