> 🚨 **SUPERSEDED/STALE (DOCMAP3, 2026-09-24; moved from `docs/PHASE3_5_TIMELINE_INTEGRATION.md`):** only the Outlook arm runs (27,305 `activity_events` with `source_type='outlook'`). The `salesforce_activity_log` and `meetings` sources are empty because their bridges were never seeded (backlog `BRIDGES-DORMANT`). Function-count remarks are obsolete (Railway).
