...
| 👤 **C4a — what promotes an owner out of `unknown`** | ✅ **DESIGNED 2026-08-31 to Scott's four constraints — [`owner-role-classification.md`](owner-role-classification.md)** (accuracy first · not a one-time determination · automate second · entity-level). ⚠️ **Accuracy-first RETIRES C12's option B** (classify + gate P0.4): suppressing an accurate determination to protect an ungated band is the wrong trade — **P0.4's missing value gate is P0.4's defect (C4f).** ⚠️ **The vocabulary cannot express the most valuable state — 3,795 orgs owned before and hold nothing now (2,784 still `unknown`), and repeat sellers are the business model. `former_owner` is required for accuracy.** Derivation is safe: **churn is 3 ended / 1 started in 90 days.** Arms are recorded facts only; **no lexical classifier**. Sizing evidence: [`C12_C4a_DECISION_BRIEF_2026-08-31.md`](../audits/C12_C4a_DECISION_BRIEF_2026-08-31.md) — 3,217 unknown orgs hold a current asset; 2,949 would enter P0.4 (555 → ~3,500). **Four open questions in §6.** |
