...
| 👤 **C4a — what promotes an owner out of `unknown`** — ✅ **SIZED 2026-08-31, see [`C12_C4a_DECISION_BRIEF_2026-08-31.md`](../audits/C12_C4a_DECISION_BRIEF_2026-08-31.md).** **3,217 `unknown` orgs hold a current asset** — a recorded fact; top 16 by rent are Easterly, NGP, USAA, US Fed Properties Trust, Government Properties Income Trust, Elman, Piedmont REIT; **6 brokerages, 3 placeholders, 124 not-prospected** across the whole set. ⚠️ **But 2,949 of them would enter P0.4, taking it 555 → ~3,500** — a 6× flood of the band that already makes the queue 57% data-completion work, because **P0.4 has no value gate.** ⚠️ **The deal-timing bands are unaffected** (C6 removed the role from them) and the brief barely moves (C8's resolved-owner arm already admits most). **Three options with measured consequences in the brief; the floor is a business judgement, which is why it stays Scott's.** | **Scott's, doctrine not code.** Recorded facts available, none adopted: portfolio shape · `purchases` edges (repeat investor vs one-off — his own distinction, already modelled) · `is_operator_not_owner` (P113) · deed/B5 party roles |
