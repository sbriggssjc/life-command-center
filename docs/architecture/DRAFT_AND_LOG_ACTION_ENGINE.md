> 🚨 **SUPERSEDED/STALE (DOCMAP3, 2026-09-24; moved from `docs/DRAFT_AND_LOG_ACTION_ENGINE.md`):** go-live was verified 2026-07-20 (see the bottom section), so "inert until" in the Activation section no longer holds. The "comments in BOTH modes" follow-up is done (`api/_shared/salesforce.js` `commentDefault` / `cleanActivityComment`).
