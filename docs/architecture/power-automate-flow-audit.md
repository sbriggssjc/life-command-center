...
## Companion documents
- `power-automate-observability-standards.md` — the seven-control reliability/observability bar every flow is held to, plus the per-flow compliance matrix ("locked" 2026-05-14).
- `lcc-microsoft-salesforce-pipeline-gap-analysis.md` — structural gaps in the LCC ↔ Microsoft/Salesforce integration architecture and the prioritized backlog of flows/processes that do not yet exist.
- `flows/FLOW_CHANGES_LOG.md` — the change ledger (every change, with validation evidence and rollback).
...
| OutlookCalendar-LifeCommandCenterSync | `OutlookCalendar-LifeCommandCenterSync_20260512134742.zip` | `Recurrence` (hourly) | Merge multi-calendar events and sync consolidated payload to Supabase edge endpoint. **Assessed healthy + HARDENED 2026-05-14** (Tasks #7/#11): all runs Succeeded in 28d; added explicit Exponential retry (4×PT10S) and `correlation_id`+`schema_version` to the POST payload. Flow ID `74ba8f8d-6454-4753-8cb8-524605129d6c`. | `flows/outlookcalendar-lcc-sync.md` + `FLOW_CHANGES_LOG.md` Tasks #7/#11 | Active (healthy, hardened) |
| LCC-PersonalCalendarSync | `LCC-PersonalCalendarSync_20260512134721.zip` | `Recurrence` (hourly) | Pull personal calendar events and sync to Supabase edge endpoint. **Assessed healthy + HARDENED 2026-05-14** (Tasks #7/#10): all runs Succeeded in 28d; added explicit Exponential retry (4×PT10S) and `correlation_id`+`schema_version` to the POST payload. Edge endpoint confirmed idempotent on event-id (Task #14). Flow ID `99dd28dc-c627-4188-898c-b60669e0c270`. | `flows/lcc-personal-calendar-sync.md` + `FLOW_CHANGES_LOG.md` Tasks #7/#10 | Active (healthy, hardened) |
...
| CompleteSFTask | `CompleteSFTask_20260512134535.zip` | HTTP Request (`manual`) | Find open Salesforce Tasks and conditionally complete/update by action. **FULLY FIXED 2026-05-14** (Tasks #6 Part B.1 + #12): OData injection closed via `EscapeSubject` Compose; Condition made null-safe; and the systemic `?['records']`→`?['value']` key bug corrected across all 7 references in the Condition + the 5 Switch-branch actions (the flow's entire success path was previously non-functional). 0 runs/28d. Flow ID `06b7b1dc-f917-4970-b075-7dd7fcef56c1`. No residual items. | `flows/complete-sf-task.md` + `FLOW_CHANGES_LOG.md` Tasks #6 B.1 / #12 | Active (FULLY FIXED) |
...
| Log Activity to SF from LCC | `LogActivitytoSFfromLCC_20260512135623.zip` | HTTP Request (`manual`) | Create Salesforce Task activity record from LCC-triggered payload. **HARDENED 2026-05-14** (Tasks #6 Part B.3 + #13): trigger Request Body JSON Schema added with a `required` array (strict payload validation — PA now 400s malformed payloads); `AuditLog` Compose added before `Create record` emitting `{correlation_id, source, schema_version, received_payload}`. Structure now `manual → AuditLog → Create record → Response`. 0 runs/28d. Flow ID `6700bdfc-3bbd-4c85-a85c-e9660042aab1`. Residual: `X-LCC-Key` request-auth Condition — blocked on needing the secret value (recommend a Power Platform environment variable). | `flows/log-activity-to-sf-from-lcc.md` + `FLOW_CHANGES_LOG.md` Tasks #6 B.3 / #13 | Active (hardened; request-auth gap documented) |
...
|---|---|---|---|---|---|
| P0 | Security | Exported definitions include embedded bearer/service-role credentials in flow actions. | Rotate exposed keys immediately, move secrets to secure references, re-export and verify redaction. | Platform + Flow Owners | Open — only remaining open item; tracked as the single RED in the observability matrix. |
| P1 | Reliability | 1-minute recurrence polling lacks documented per-kind idempotency/retry/dead-letter policy. | Add explicit retry ceilings, dead-letter routing, and idempotency checks per `kind`. | Flow Owner | Standard set 2026-05-14 (`power-automate-observability-standards.md`, controls 3-4); retro-fit is Waves 2-4 of the rollout. |
| P1 | Contract Governance | No explicit shared schema version + strict validation branch across request/queue payloads. | Introduce `schema_version`, enforce validation branch, fail closed for unknown contracts. | LCC + Flow Owner | Standard set 2026-05-14 (controls 2, 5); applied to Log Activity to SF + both calendar flows; portfolio retro-fit pending. |
| P2 | Observability | Incomplete standardized telemetry mapping across flow run -> queue row -> endpoint response -> final state. | Add correlation logging fields and runbook queries for traceability. | LCC + Flow Owner | Standard set 2026-05-14 (control 1); central dead-letter/`flow_health` plane identified as a missing build — see gap analysis backlog item #2. |

> Note: the structural gaps beyond this initial matrix — Salesforce event-driven inbound, object-coverage gaps, the missing central dead-letter plane, bidirectional To Do/Calendar, no non-prod environment — are catalogued with a prioritized backlog in `lcc-microsoft-salesforce-pipeline-gap-analysis.md`.
