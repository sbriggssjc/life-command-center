> 🚨 **SUPERSEDED/STALE (DOCMAP3, 2026-09-24; moved from `docs/PHASE3_OUTLOOK_CALENDAR_BRIDGES.md`):** only the `outlook.messages` half is live; `calendar.events` never ran, and `v_contact_engagement` does not exist (measured 2026-09-24). ⚠️ Do NOT copy the `/api/me` id into `X-LCC-Source-User-Id`: that is the `lcc_users` id, and it FK-failed 10,470 `email_bodies` writes (P116); it is now normalised by `resolveSourceUserId`. Routing is Railway `server.js`, and `unified_contacts` lives on LCC Opps under `CONTACTS_HUB=ops`. Canonical pages are `correspondence-ingestion-design.md` and `calendar-system-status.md`.
