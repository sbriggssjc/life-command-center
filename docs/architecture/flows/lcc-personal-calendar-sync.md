...
- **Hardened 2026-05-14 (Task #7 follow-up):** HTTP POST retry policy set to explicit Exponential (count 4, interval PT10S); body enriched with `correlation_id` (guid()) + `schema_version` for per-run traceability.
