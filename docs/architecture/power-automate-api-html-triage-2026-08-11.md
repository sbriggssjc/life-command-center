...
Update 2026-08-11 16:08 local: Scott exported updated copies of both roster flows after editing the HTTP action URI to the canonical host. The definitions now point to `tranquil-delight-production-633f.up.railway.app`, and unauthenticated empty POST probes to both canonical routes return JSON `401`, confirming the routes are mounted and auth-gated rather than returning HTML or route-missing JSON.

Update 2026-08-11 16:08 local run-history evidence: both edited roster flows reached Railway and returned `200` JSON. `SF Deal Team -> LCC Roster` reported `ok:true`, `total=200`, `tb_members=200`, `edges_existing=200`, `skipped_no_deal=0`, confirming the team roster path is healthy and idempotent. `SF Deal Contacts -> LCC Roster` reported `ok:true`, `total=7213`, `edges_created=0`, `edges_existing=0`, `skipped_no_deal=7213`, confirming routing/auth are fixed but the contact-role payload does not currently match LCC's deal backbone.

Source-query check: the team roster flow reads `OpportunityTeamMember` scoped to the four Team Briggs Salesforce user IDs, which matches the handler's contract. The contacts roster flow reads `OpportunityContactRole` with only `IsDeleted eq false`, so it sends a broad org-level role list and relies on LCC to discard non-backbone deals. The run evidence shows that source currently contributes no LCC deal-party edges.
...
| P0 remediated export | SF Deal Team -> LCC Roster | `9879c0fd-2dc0-4304-a82b-d68de3fcc991` | owner-access required | `HTTP` | POST | `https://tranquil-delight-production-633f.up.railway.app/api/pipeline/ingest-deal-parties` | Same as above | Updated export verified at `C:\Users\scott\life-command-center\private\power-automate\exports\production\2026-08-11\SFDealTeam→LCCRoster_20260811160520.zip`; SHA-256 `3e6495930c9b370e8fddb6b4772ed73cdefd165424a2dd7e3e1de291159f261e`. Body/method/runtime unchanged. | remediated, latest run pending owner review |
| P0 remediated export | SF Deal Contacts -> LCC Roster | `a50d3f56-3891-4d8f-8636-b9b09e58c2ee` | owner-access required | `HTTP` | POST | `https://tranquil-delight-production-633f.up.railway.app/api/pipeline/ingest-deal-contacts` | Same as above | Updated export verified at `C:\Users\scott\life-command-center\private\power-automate\exports\production\2026-08-11\SFDealContacts→LCCRoster_20260811160810.zip`; SHA-256 `fd2ce647c05e3e67cf71d137f4e8e4ad875d6191d0fb9df6524781a7633b390e`. Body/method/runtime unchanged. | remediated, latest run pending owner review |
| P0 accepted | SF Deal Team -> LCC Roster | `9879c0fd-2dc0-4304-a82b-d68de3fcc991` | owner-access required | `HTTP` | POST | canonical endpoint above | JSON `{ok:true,...}` | Run 2026-08-11 11:03:59-11:04:41 local, status `Succeeded`, HTTP 200 JSON: `total=200`, `tb_members=200`, `edges_existing=200`, `skipped_no_deal=0`. | routing/auth fixed; business accepted |
| P1 business-state follow-up | SF Deal Contacts -> LCC Roster | `a50d3f56-3891-4d8f-8636-b9b09e58c2ee` | owner-access required | `HTTP` | POST | canonical endpoint above | JSON `{ok:true,...}` with useful resolved/edge counts | Run 2026-08-11 11:08:33-11:08:34 local, status `Succeeded`, HTTP 200 JSON: `total=7213`, `resolved=0`, `edges_created=0`, `edges_existing=0`, `skipped_no_deal=7213`. This is valid JSON but a no-op against the current LCC deal backbone. | D |
...
| `POST https://tranquil-delight-production-633f.up.railway.app/api/pipeline/ingest-deal-parties` | 401 | `application/json; charset=utf-8` | `{"error":"Unauthorized — invalid or missing Bearer token"}` |
| `POST https://tranquil-delight-production-633f.up.railway.app/api/pipeline/ingest-deal-contacts` | 401 | `application/json; charset=utf-8` | `{"error":"Unauthorized — invalid or missing Bearer token"}` |
...
- D, valid JSON business-state no-op: confirmed for `SF Deal Contacts -> LCC Roster` after host remediation. The flow succeeds at the transport layer, but every submitted contact role skipped because its `OpportunityId` did not resolve to a synced `bd_opportunities.sf_opp_id` row.
...
| P1 | `SF Deal Contacts -> LCC Roster` broad no-op | Either pause the flow or rescope the Salesforce source query before keeping it scheduled/available. Current query is `OpportunityContactRole` with `IsDeleted eq false`, which produced 7,213 skipped rows and zero edges. | Optional code hardening: return `ok:false` or `warning` when `total > 0` and `skipped_no_deal == total`, so PA cannot mistake a full no-op for healthy ingestion. | No deploy needed to pause/rescope the PA query; code warning requires Railway deploy. | Restore current export if a scoped query misses expected contacts. | Next run should show `resolved > 0` or `edges_existing/created > 0`; otherwise the flow should visibly report a no-op warning/failure. |
