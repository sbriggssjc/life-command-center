> 🚨 **SUPERSEDED/STALE (DOCMAP3, 2026-09-24; moved from `docs/T4C_RECOVERY_on_market_backfill.md`):** the "Item 3 not done — charts still read `listing_date`" note is obsolete: `on_market_date` is the canonical market-entry date on dia and gov (root `CLAUDE.md`, Domain-DB invariants). The backfill counts are 2026-06-24 snapshots; re-measure before quoting.
