...
| W1.4 | LLC research revival (requeue FL deferred; OpenCorporates decision) | 🟨 2026-07-29 | Cowork half done: **FL deferred = 0 on BOTH DBs** (FL backlog already drained once the Sunbiz adapter shipped — requeue is a no-op; audit's premise stale). Deferred-by-state distribution: dia top = unknown 237, CA 143, TX 98, GA 72, IL 59, NC 50; gov top = unknown 455, TX 46, CA 38, VA 33, AZ 33. failed = 0 both. **DECISION PENDING (Scott):** OpenCorporates key (~4.1k calls clears the whole ~2,040-row backlog incl. the 692 unknown-state rows one month of a paid plan could cover, then downgrade) vs building TX+CA bulk mirrors (covers ~325 rows, leaves unknowns unsolved). Recommendation: one paid OpenCorporates month to drain, keep FL mirror for steady state. Code half (bulk requeue-on-new-adapter tick + weekly failed-retry) waits on the decision. |
...
- **2026-07-29 (Cowork, session 3 — independent verification of W0.3f/W0.7/W0.8/W0.3h/W1.1):** All live-DB claims re-verified from Cowork/Supabase MCP: LCC staged_intake_feedback=3,729 (3,728 approved), v_matcher_accuracy_recent=34 band rows, matcher-accuracy-rollup cron active, feature_flags_registry=22; dia agency_debt_programs schema=archive, merge_dialysis_dup_property refs=0, anon-executable secdef=0, manifest=36; gov anon-executable secdef=6 (=exception list), lease_lifecycle_events schema=public (correct — pending W0.3h deploy + one live promotion before re-archive), manifest=16. ALL PASS. W1.4 Cowork half executed same session (FL requeue no-op — already drained; state distribution captured; OpenCorporates decision queued for Scott). Wave 0 effectively closed: remaining are W0.6 (Scott dashboard), W0.3h re-archive trigger (post-deploy), W0.3i + ingest_agency_debt.py retirement (riding with W1.5), W0.5b (parked). Stale duplicate W0.3h/W0.3i rows deduped from the Wave 0 table (statuses live in rows 41-43's versions).
