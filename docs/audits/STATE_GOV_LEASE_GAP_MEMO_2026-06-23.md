> 🚨 **SUPERSEDED/STALE (DOCMAP3, 2026-09-24; moved from `docs/STATE_GOV_LEASE_GAP_MEMO_2026-06-23.md`):** remediation shipped 2026-06-23: classifier miss-rate 54% → 0%, the TX TFC engine is live (the monthly `state-lease-ingest` workflow in government-lease), and SF deal→sale promotion was built behind `SF_DEAL_SALE_PROMOTION`. The unticked close-out list was never reconciled. State-lease truth lives in government-lease `CLAUDE.md` §21/§27.
