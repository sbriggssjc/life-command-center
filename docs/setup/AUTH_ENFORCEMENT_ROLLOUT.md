> 🚨 **SUPERSEDED/STALE (DOCMAP3, 2026-09-24; moved from `docs/AUTH_ENFORCEMENT_ROLLOUT.md`):** enforcement is already ON: live `/api/diag?kind=auth-ready` read `lcc_env: production, enforcing: true` on 2026-08-29 (CURRENT-STATE auth row). Read this doc as the rollback and diagnosis reference. An unauthenticated probe returning `would_pass_in_production:false` is now expected.
