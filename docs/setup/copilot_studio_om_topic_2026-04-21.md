...
> **DO NOT paste `copilot/openapi.yaml` directly into Power Platform** —
> that file only defines the 5 new operations and would wipe the 38 existing
> ones. Use the merged spec below instead.

The merged spec is generated by `scripts/merge_copilot_openapi.py` and lives
at `docs/setup/lcc-copilot-openapi-merged.json`. It combines the existing
38 operations (GetDailyBriefing, GetHotContacts, SearchEntities, etc.) with
the 4 new v1.1.0 operations (intakeStageOm, intakeFinalizeOm,
contextRetrieveEntity, memoryLogTurn) — 42 operations total.

To refresh the connector:

1. Regenerate the merged spec if needed:
   ```
...
4. Paste the contents of `docs/setup/lcc-copilot-openapi-merged.json` in full.
5. Verify both the existing operations AND the 4 new ones appear under
   _Definition_:
...
6. Save → Update connector → Test with a known intake_id to confirm 200s.

If Power Platform reports a validation error, the most common culprit is
nullable enum members or `type: [string, "null"]` syntax in custom edits
on top of the merged file. All nullable fields in the merged spec use
the compliant `"nullable": true` form.
