...
1. Open Power Platform → Custom Connectors → _LCC Deal Intelligence_.
2. Edit → **Swagger Editor**.
3. Paste the contents of `copilot/openapi.yaml` (updated 2026-04-21) in full,
   replacing the existing spec.
4. Verify the following operations now appear under _Definition_:
   - `copilotAction` (POST `/api/copilot/action`)
...
5. Save → Update connector → Test with a known intake_id to confirm 200s.
