> 🚨 **SUPERSEDED/STALE (DOCMAP3, 2026-09-24; moved from `docs/runbooks/sjc_broker_contact_sync.md`):** UNBUILT as of 2026-09-24. Dia `salesforce_contacts` holds 0 of the 32 `v_sjc_broker_contact_ids`, and on gov `salesforce_contacts` / `v_sjc_deal_book` do not exist. The gov schema is now owned by government-lease (LCC `supabase/migrations/government/*` is historical), so the gov fan-out must be written and applied there. "Flow 6" also names other flows; disambiguate before building. Backlog `SJC-BROKER-SYNC`.
