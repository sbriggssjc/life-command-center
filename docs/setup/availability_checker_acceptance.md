> 🚨 **SUPERSEDED/STALE (DOCMAP3, 2026-09-24; moved from `docs/availability_checker_acceptance.md`):** dia `available_listings` has no `url_status`; the function writes `is_active`, so the dia provenance rung does not match its writer (backlog `PR5c-avail-field`). PR5c's fix to the function is NOT deployed (`PR5c-deploy`). Check `list_edge_functions` `updated_at` before trusting any provenance check below.
