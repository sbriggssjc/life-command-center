> 🚨 **SUPERSEDED/STALE (DOCMAP3, 2026-09-24; moved from `docs/CONTACT_ENRICH_ADAPTERS.md`):** 2026-06-20 grounding. The web-search proxy (Phase D, `owner-contact-websearch`) is PAUSED and must not be activated (root `CLAUDE.md`). The cross-ref resolver has since shipped (`owner-cross-reference.js`). SOS-direct now routes via the government-lease `sos-proxy` (`W9_1_SOS_DIRECT`, still off). Re-measure counts before acting.
