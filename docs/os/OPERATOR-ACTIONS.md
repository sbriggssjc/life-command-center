...
| **ENTC-confirm** | **Confirm the 15 same-person merges, row by row.** `select * from v_lcc_entities_c_review_merge_plan order by basis, winner_name;` then per row the plan's own `confirm_sql` (`lcc_merge_entity(loser, winner)`). ⚠️ Read `winner_decided_by` first — the P195 winner rule degenerates on contact-only rows (all ownership tiers 0), and on at least 3 rows the older, better-named side lost (`Frank D. Johnson`); swap winner/loser where judgement says so. ⚠️ **Reverse only with `lcc_unmerge_entity`** — `lcc_p195_unmerge` strands duplicate edges while reporting `restored`. |
