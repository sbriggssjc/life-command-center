...
|---|---|---|
| **OCR1** | **Run the local-OCR bake-off on the workstation or GaryBuilt box** — the harness is built (`scripts/ocr-bakeoff.mjs`) but the sandbox cannot reach the PDFs or Supabase. Sequence: `pip install surya-ocr paddleocr` → `node scripts/ocr-bakeoff.mjs --self-test` → `--fetch-baselines 336,431,425,327,255,386,343,299,436,228` → put each PDF at `bakeoff/<id>/source.pdf` (arm B, no baseline: 319, 320, 200, 407, 140) → `--run --model real`. Paste `agreement.md` into `responses/`. | **The whole OCR1/OCR1b decision** — whether the page-cap class of problem (DOC8→DOC18) goes away and whether client leases stop going to Google. ⚠️ Read `both_null` per row before believing a rate; a high share means the sample did not discriminate. |
