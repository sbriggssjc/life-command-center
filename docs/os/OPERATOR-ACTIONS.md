...
| **UX13a** | **Configure the Outlook body-sweep Power Automate flow for Kelly, Nate and Sarah's mailboxes** (today only Scott's is synced: `email_bodies` 30,357 vs 0 / 0 / 0). ⚠️ Each needs the `X-LCC-Source-User-Id` set to that person's **`public.users`** id (P116 — not the `lcc_users` id). | The voice corpus, deal attribution, Team Pulse touchpoints and the call sheet for three of four team members — "Kelly's touchpoints are stale" was this, not a code defect. |
...
| **UX39 · UX41** | **Two removals from your app review were REFUSED on measurement (UX-T0 §2) — your call with the numbers.** *National ST*: the tab is one button, but it is the **only** route to `renderNatlStCapitalMarkets` and the RCA upload card that feed the Single-Tenant quarterly book (18 live `cm_natl_st*` views, 480 rows). Delete = orphan that ingestion path. *All Other*: not duplicative — Prospects is a search box that renders nothing until you type; All Other is the only browsable list, and it holds **6,245** opportunities, the largest domain bucket. Options: keep both (relabel National ST as the ST-book intake); or build a list mode into Prospects first, then fold. |
...
| **PR2-gov** | **Run the gov parcel-stat backfill?** `node scripts/pr2-backfill-sidebar-parcel-stats.mjs --domain government --apply` — 1,527 `costar_sidebar` parcel rows with 0 stats, ceiling 1,230 SF / 1,192 year built / 1,155 lot / 310 zoning. Same fixed parser, reversible by batch tag. Left un-run on 2026-09-02 rather than widen a dia-scoped change; nothing blocks it. **Also confirm the Railway redeploy carries `98248e18`, `68ede28c` AND `06a3ee5d`** (`/version`) — until it does, new sidebar captures still drop the stats (PR2) a provenance write failure is still silent (PR12's JS half), and five provenance callers still send an invalid `target_database` (PR5c). |
| **PR2-gov** | **Run the gov parcel-stat backfill?** `node scripts/pr2-backfill-sidebar-parcel-stats.mjs --domain government --apply` — 1,527 `costar_sidebar` parcel rows with 0 stats, ceiling 1,230 SF / 1,192 year built / 1,155 lot / 310 zoning. Same fixed parser, reversible by batch tag. Left un-run on 2026-09-02 rather than widen a dia-scoped change; nothing blocks it. ~~Also confirm the Railway redeploy carries `98248e18`~~ ✅ **Confirmed 2026-09-02 ~18:20 UTC: `/version` = `35528de9` (a descendant of `98248e18`)** — new sidebar captures now carry the stats. ⚠️ Use a cache-busting query param on `/version`; a plain fetch returned a stale body. |
