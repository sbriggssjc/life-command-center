...
| **ENTC-confirm** | **Confirm the 15 same-person merges, row by row.** `select * from v_lcc_entities_c_review_merge_plan order by basis, winner_name;` then per row the plan's own `confirm_sql` (`lcc_merge_entity(loser, winner)`). ⚠️ Read `winner_decided_by` first — the P195 winner rule degenerates on contact-only rows (all ownership tiers 0), and on at least 3 rows the older, better-named side lost (`Frank D. Johnson`); swap winner/loser where judgement says so. ⚠️ Reverse with `lcc_unmerge_entity`. (`lcc_p195_unmerge` was FIXED 2026-09-03 and is no longer unsafe — but it reverses a P195 *batch tag*, not an individual merge, and the 66 open P195 batches are the only thing it owns.) |
| **junk80-apply** | **Seed the 80 junk-named person entities into the junk review lane?** Dry run first: `GET /api/admin?action=junk80-seed` — it returns the split (41 `sweep_candidate` proposing `dismiss`, 39 holds proposing `uncertain`/`rename`) and the projection (35 mailboxes freed, 49 identities detached, **0 relationships touched**). Apply with `POST /api/admin?action=junk80-seed&apply=true`. Seeding only creates proposals; each retire is still your confirm, and `unstampMisparseMember` makes it reversible via `junk_review_batch`. ⚠️ **Do not bulk-confirm** — 6 rows ARE their own mailbox's person and 27 carry a Salesforce identity; those seed `uncertain` on purpose. Evidence: `ENTC_JUNK80_AND_P195_UNMERGE_2026-09-03.md`. ✅ **Deploy confirmed** — live `/version` = `5b3b1227` (2026-09-03), so the seed handler, the mint gate and the un-stamp keying are all running; nothing blocks the dry run. |
