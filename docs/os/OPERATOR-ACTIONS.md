...
| **PR5d-a** | **Does your CoStar session reach the loan/CMBS sub-page, and does the subscription expose the servicer report?** The scanner, the writer and the URL match are all live and correct — the page has just never been captured (0 of 2,219 loan rows carry `costar_loan_id`). If you can reach it, one Extract on a gov CMBS property fills watchlist / delinquency / DSCR / special-servicing, which is the ONLY input to R54's `is_distressed` ranking arm (**0 of 178 gov watch rows today**). If the subscription does not expose it, say so and the 94 rungs get retired instead. |
