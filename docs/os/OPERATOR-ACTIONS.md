...
|---|---|---|
| **OCa-live-verify** | **After the OC-a branch merges (✅ PR #2298) and Railway redeploys** — ⚠️ 2026-09-11: not yet; the connected LCC MCP still lacks `log_operator_note`/`get_operator_inbox`, 0 notes in `operator_notes` — (both `tranquil-delight` + the standalone MCP service — OC-a touched `api/` and `mcp/server.js`): post one operator note through each live channel (the in-app Note button, `log_operator_note` via Claude Code/Cowork, and an `LCC-Note`-categorized Outlook email if the tagged-comm PA flow is confirmed alive), then run `GET/POST /api/operator-triage-tick` once with `?force=1` and confirm each note is classified/routed and appears in a fresh `docs/os/OPERATOR-INBOX.md` render + `get_operator_inbox`. | Whether OC1/OC2/OC3 actually work live, not just in unit tests — nothing in the repo can confirm this without a live deploy. |
| **OCa-flag-flip** | **Once OCa-live-verify passes**, **insert** the `OPERATOR_NOTE_TRIAGE` row (it does not exist in `feature_flags_registry` as of 2026-09-11 — there is nothing to flip yet) with `state='on'` in `feature_flags_registry` (LCC Opps) and pick a schedule for the triage tick (a pg_cron job posting to `/api/operator-triage-tick`, e.g. every 15–30 min, mirroring the P133 `lcc-ownership-chain-draft` cadence pattern). | Turns the triage tick from a manually-invoked endpoint into an actual funnel — without this, notes accumulate `disposition='open'` forever. |
| **OCa-pa-note-category** | **Confirm the Outlook `LCC-Note` category actually reaches `intake-tagged-comm.js`.** The regex bug that dropped it is fixed in code, but that only helps if the underlying Power Automate category-assigned trigger is still firing at all — the broader tagged-Outlook flow reads "flag-on but dormant, 0 rows in 30 days" per the 2026-09-11 reconcile, a SEPARATE question this build could not answer from the sandbox. Tag one test email `LCC-Note` and check whether an `operator_notes` row lands. | Whether the Outlook channel is real or just code that has never been exercised. |
| **OCa-teams-flow** | **Build a Power Automate flow** that POSTs a Teams message to `POST /api/operator-notes` with `{channel:'teams', raw_text, context:{teams_message_id, channel_id}}` and header `X-PA-Webhook-Secret: <PA_WEBHOOK_SECRET>` (same secret already used by the tagged-Outlook flow). No bot was built — this is the one remaining OC1 channel from the spec. | The Teams channel of the operator funnel — currently endpoint-ready with no producer. |
| **OCa-inbox-hook-creds** | **Put `OPS_SUPABASE_URL`/`OPS_SUPABASE_KEY` into whatever `.env.local` template Claude Code sessions load**, so the now-wired `session-start.sh` inbox refresh actually reaches the DB instead of no-opping. | The hook is wired (`.claude/hooks/session-start.sh` calls `render-operator-inbox.mjs --write`) but every sandbox session lacks the creds, so `docs/os/OPERATOR-INBOX.md` never actually refreshes yet. |
| **MBa-hold** | ✅ **Hold can lift (2026-09-11, Cowork dry-run on deployed `78082f46`): P-SQL `gaps:[]`, CMS gate working; P-RSS reaches Ollama.** Flipping now writes facts nobody renders yet — safe; MB-b §0 then supersedes the fragmented operator bands. Scott's call: flip now (`update feature_flags_registry set state='on' where flag in ('MARKET_BRIEF_PSQL','MARKET_BRIEF_PRSS');`) or let MB-b flip after its cleanups. ~~Do not flip `MARKET_BRIEF_PSQL` / `MARKET_BRIEF_PRSS` yet.~~ ⚠️ **Still holding after MB-a2 (2026-09-11): the CMS facts would publish a Jan-2026 census as today's fact (MB1d); MB-a3 fixes that and does the flip.** Crons are active and POST daily against the OFF flags (named skips, harmless). Cowork's 2026-09-11 live check found the P-SQL source queries broken (missing columns) and the CMS count truncated (limit 1000 vs 6,695 rows). MB-a2 applies the migration, fixes the sources, and verifies. Redeploying both Railway services after MB-a2 also ships OC-a's pending code. | ⏳ 👤 |
| **OCR1** | ⚠️ **The box run (19:00 UTC) graded NO GPU engine — two install steps missed.** (1) `run_check()` said *"1 CPU"*: install the GPU wheel for the CUDA `nvidia-smi` shows (`pip uninstall paddlepaddle` first; then `pip install paddlepaddle-gpu` from the index for that CUDA — see paddlepaddle.org.cn/en/install) and re-run `run_check()` until it names the GPU. (2) surya: `python -c "from surya.settings import settings; print(settings.SURYA_INFERENCE_BACKEND, type(settings).model_fields['SURYA_INFERENCE_BACKEND'])"` — if a non-vllm value (llama.cpp) is allowed, `$env:SURYA_INFERENCE_BACKEND='<that value>'` and run without Docker; otherwise Docker Desktop with GPU. Then: `node scripts/ocr-bakeoff.mjs --run --model real --engines surya,paddleocr,tesseract --control self`. ✅ EXT2 floor re-run DONE 2026-09-03 (7/8 source agreement; 299 resolved); ✅ EXT2a SHIPPED 2026-09-03 (255's schedule-blend double count fixed). | Whether a GPU engine reaches the DocAI floor (OCR1b / DOC14 decision), and whether EXT1 moved the floor. |
