...
|---|---|---|
| **OCR1** | ✅ Two runs done on the workstation (tesseract only: 80% vs a 93% model floor — §5 row 2). **The deciding run is on the GaryBuilt box**: copy `bakeoff/` there (15 PDFs + baselines), install surya-ocr (Docker for its VLM server) and the `paddlepaddle-gpu` wheel matching the box's CUDA (the CPU wheel hits the same oneDNN bug), then `node scripts/ocr-bakeoff.mjs --run --model real --engines surya,paddleocr,tesseract --control self`. Read the floor table first, then each engine's `rate − self`. | Whether a GPU engine reaches the DocAI floor. If neither does, OCR1b is refused and DOC18 stands; if one does, OCR1b wires it and DOC14 closes. |
