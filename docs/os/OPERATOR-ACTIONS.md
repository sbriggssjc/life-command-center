...
|---|---|---|
| **OCR1** | **Run the local-OCR bake-off on the workstation or GaryBuilt box** — the harness is built (`scripts/ocr-bakeoff.mjs`) but the sandbox cannot reach the PDFs or Supabase. ✅ Engines installed and 15/15 PDFs staged 2026-09-02; ⚠️ the first run was a silent no-op (Windows main-guard bug, fixed — `CLAUDE.md`). **Re-run after merging:** `node scripts/ocr-bakeoff.mjs --self-test` (must print ~15 ✔ lines — silence means it did not run) → `--fetch-baselines 336,431,425,327,255,386,343,299,436,228` → `--run --model real`. Paste `agreement.md` into `responses/`. | **The whole OCR1/OCR1b decision** — whether the page-cap class of problem (DOC8→DOC18) goes away and whether client leases stop going to Google. ⚠️ Read `both_null` per row before believing a rate; a high share means the sample did not discriminate. |
| **B6e-ci-required-check** | **Dialysis → Settings → Branches → `main` → require status checks `Run Tests` + `Build Check`.** ⚠️ Do it together with the `paths-ignore` docs-only fix (a skipped run reports no status and would block every docs-only PR) — ask the chat for that PR first, then flip. | **The whole B6 CI arc.** The pytest line is unmasked and green on `main` (PR #7393), but without this toggle a red suite fails a job and blocks nothing — #7393 itself merged 8 s after its test job started. |
