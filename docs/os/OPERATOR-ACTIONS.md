...
|---|---|---|
| **OCa-live-verify** | **After the OC-a branch merges and Railway redeploys** (both `tranquil-delight` + the standalone MCP service — OC-a touched `api/` and `mcp/server.js`): post one operator note through each live channel (the in-app Note button, `log_operator_note` via Claude Code/Cowork, and an `LCC-Note`-categorized Outlook email if the tagged-comm PA flow is confirmed alive), then run `GET/POST /api/operator-triage-tick` once with `?force=1` and confirm each note is classified/routed and appears in a fresh `docs/os/OPERATOR-INBOX.md` render + `get_operator_inbox`. | Whether OC1/OC2/OC3 actually work live, not just in unit tests — nothing in the repo can confirm this without a live deploy. |
| **OCa-flag-flip** | **Once OCa-live-verify passes**, set `OPERATOR_NOTE_TRIAGE = 'on'` in `feature_flags_registry` (LCC Opps) and pick a schedule for the triage tick (a pg_cron job posting to `/api/operator-triage-tick`, e.g. every 15–30 min, mirroring the P133 `lcc-ownership-chain-draft` cadence pattern). | Turns the triage tick from a manually-invoked endpoint into an actual funnel — without this, notes accumulate `disposition='open'` forever. |
