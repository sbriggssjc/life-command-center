...
| ~~V7~~ | ⚠️ **ROOT-CAUSED 2026-08-27 — the Analyst's Take pipeline is BROKEN, and it is a config gap, not a code defect.** Cron 240 fired at **10:18:00** and returned **HTTP 400**: `{"ok":false,"error":"Could not resolve workspace. Set X-LCC-Workspace or LCC_DEFAULT_WORKSPACE_ID."}`. Today's `briefing_intel_snapshot` row exists (created 10:00:16) with `analyst_take` **NULL** — confirming the 2026-08-26 774-char take was a **manual one-shot** (`generated_at` 20:51), never the pipeline. **Fix: set `LCC_DEFAULT_WORKSPACE_ID` in the Railway env, or have `lcc_cron_post` send `X-LCC-Workspace` for job 240.** Then verify a take lands with `generated_at` inside the 10:18 window. ⚠️ **Two SEPARATE faults surfaced in the same chain and must not be conflated:** `/api/daily-briefing` returns **401 Unauthorized** (missing/invalid Bearer token), and `briefing-intel-snapshot` still warns *"Anthropic API 400: credit balance too low"* — the known cloud-billing issue that the on-box take exists to route around. | 🔴 👤 | measured 2026-08-27 11:15 UTC |
...
| N3e | ~~95 parked Tier 0 cards are stuck PERMANENTLY~~ **✅ DONE 2026-08-27 (prompt 196 Unit 2) — and they are mostly parked CORRECTLY.** Every parked card now carries a `park_reason` + both compared strings (`v_lcc_tier0_owner_contact_lane_triage` / `v_lcc_tier0_park_watch`, and the `GET /api/tier0-auto-attach-tick` dry run). Live: **146 parked / 105 owners / $180.3M** = `employer_on_file_differs` 76 cards / 67 owners / $96.3M (the gate working — this is the slice N3e's "$98M" meant), `no_employer_on_file` 68 / 56 / $132.3M, `employer_not_comparable` 2 / 2 / $1.9M. Sponsor-shaped parks are routed as `v_lcc_tier0_sponsor_map_proposals` → a curated `lcc_owner_sponsor_domain` INSERT (one decision per family). ⚠️ **The prescribed company-string normalisation was implemented, MEASURED and REJECTED: it unparks 0 of 146.** Savlan fails at character 8 (`savlancc` vs `savlanca`), not on the www/com noise. ⚠️ **The naive sponsor detector reads ~25% precision** (person given names: George Kurz ← George's Inc; place words: MAPLE HILL ← Mapletree) — three guards take it to **4 of 6, and the 4 are the top 4 by rent**. ⚠️ The un-park was NOT widened: ask 77 / auto 9 / parked 146 before and after. | ✅ | `docs/audits/P196_MERGE_REVERSIBILITY_AND_PARK_REASONS_2026-08-27.md` §7–13 |
...
| N11 | ~~`lcc_merge_entity` is IRREVERSIBLE~~ **✅ DONE 2026-08-27 (prompt 196 Unit 1).** It now snapshots the whole loser side, FOLDS `owner_contact_pivot` fill-blanks before the dedup DELETE, calls the reconcile with `p_snapshot => true`, and action-labels every P160 dedup/repoint; `lcc_unmerge_entity(loser)` is the reversal, **round-trip proven on live data** (Monaco Holdings: 16 rows before, 16 after, 0 lost, 0 new; `auto_mergeable` 3,053 → 3,053). ⚠️ **Three corrections to this row as filed:** (a) "uncorrelated EXISTS" is NOT the bug — `owner_contact_pivot` and `lcc_property_owner` are both PK'd on `entity_id`, so the predicate is equivalent to a correlated one; the bug is that it DELETES instead of FOLDING, with no ledger. (b) `p_snapshot => true` **alone would have fixed nothing that mattered** — the four P160 backrefs live in `lcc_merge_entity`, not the reconcile, and neither snapshotted them in any mode. (c) **"dormant, not armed" describes the LOOP, not the function** — `lcc_apply_fuzzy_merges` still has 0 callers, but `lcc_merge_entity` has nine human-verdict call sites and **285 merges in 30 days / 176 in 7**. ⚠️ The live round trip caught a bug review did not: P177's BEFORE INSERT trigger SKIPS a duplicate edge, so `ON CONFLICT DO UPDATE` never fires and 2 of 3 identical edges stayed on the winner while the unmerge said `restored`. **Pre-P196 backlog stated, not hidden: 2,411 tombstones, `reversible=false` for all of them.** | ✅ | `docs/audits/P196_MERGE_REVERSIBILITY_AND_PARK_REASONS_2026-08-27.md` §1–6 |
...
| **A3** | ⚠️ **PREMISE CORRECTED 2026-08-27 — this row said "route the 73 to a data-integrity lane," and that would have been the wrong build.** Measured: the 73 are dominated by **sponsor ↔ SPE** — the deed records the SPE holding title, our field records the sponsor. **Both are correct; it is a representation question, not a data error.** Concentration: **Boyd Watterson 24 of 73 (33%)** (grantees `BELTSVILLE GSA FDA, LLC`, `Boyd Bethesda III GSA, LLC`), Easterly 3, FGF 2, Brookfield 2, Blackstone 1, plus `Brent Waldman` ← `Waldman, Brent` (name order). **So the shape is ~4–8 SPONSOR decisions covering ~31+ chains, reusing `lcc_owner_sponsor_domain` (P190) and the P193 inheritance — not 73 cards.** The genuine integrity residue (e.g. `DEAMO LLC.` ← `LuLu Hsu`) is ~20–30, and should be sized before a surface is built for it. **Two hypotheses tested and refuted:** the `gsa_lease_diff` flicker (7/47 gsa chains share a prefix vs 21/27 non-gsa — the opposite of predicted) and any overlap with A2b (zero). ⚠️ **Guards: `lcc_is_spe_shell_name` under-detects place-named SPEs; a lexical sponsor detector is ~25% precise without P196's three guards; and `Boyd Watterson Global` vs `…Asset Management` may be fund vs manager — human-confirm per sponsor.** Prompt drafted: `prompts/A3-mismatch-lane-sponsor-spe-2026-08-27.md`. | 🟢 | measured 2026-08-27 |
| **A4 + A4b** | ⭐ **NEXT UP — prompt drafted `prompts/A4-retire-no-records-and-adjudicate-guarded-2026-08-27.md`.** The two unblocked buckets, in one prompt: retire the 74 unanswerable, and **measure** (not fix) which P138 guard rejected each of the 18. A4b is deliberately a measurement — if all 18 are `is_oscillating_pair` the guards are right and retiring them *is* the answer. | 🟢 | this window |
| **A4-detail** | **UNBLOCKED by A1** (`action='no_records'` — 74 tasks / 62 owners; the view can never hand you the 18, which is the point). **Auto-retire the 74 `no_transitions_on_file`** with a terminal, dated state that re-opens if new records land. ⚠️ **Only the 74 — NOT all 92.** | 🟢 | stops permanently-unanswerable items ageing into "overdue" |
...
| **A2a** | ✅ **UNBLOCKED 2026-08-27 — prompt 196 Unit 1 landed.** `lcc_merge_entity` now snapshots and folds the pivot, and `lcc_unmerge_entity(loser)` reverses a merge (round-trip proven live). ~~BLOCKED — do NOT build yet.~~ It merges duplicate entity pairs, and **`lcc_merge_entity` has no undo**: every dedup DELETE is unrecoverable, and its `owner_contact_pivot` dedup is *uncorrelated* — P195 proved it can delete a group's **only named contact**. The app window's **prompt 196 Unit 1** is fixing exactly this. **Wait for it rather than building a third merge driver.** Live population re-measured: `ambiguous_entity` = **50 tasks / 54 links / 45 distinct parties**. *(⚠️ A2's $210.6M figure and my own `sum(distinct …)` re-check disagree; neither aggregation is trustworthy per-owner — size it properly before quoting a dollar figure.)* Once 196 lands, this needs no new code: merge the pairs and **cron 244 applies the chains the same night.** | 🟡👤 | A2 response; `prompts/196-*.md` |
| **A2b** | **UNBLOCKED by A2** — 28 links blocked `repeat_transfer_unrepresentable`: one conveyance recorded on several dates (`SENTINEL SQUARE I → WASHINGTON DC VI FGF` on 2020-02, 2020-03 **and** 2020-04), surviving P131's `(from, to, date)` dedup because the DATE differs. Several are also missed name variants (`MILLENIUM TOWER` → `MILLENNIUM TOWER CORPORATION`) that the P138 `is_name_variant` guard only catches as a strict prefix extension. Fix belongs UPSTREAM in the drafter's dedup key and the P138 guard, not in A2 — A2 blocks and surfaces the alternate dates rather than guessing which record is real. ⚠️ Overlaps A3's `gsa_lease_diff` flicker hypothesis; test both together. | 🟢 | ~12 tasks, and it is a PRODUCER fix that also helps A3 |
