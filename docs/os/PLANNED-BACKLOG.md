...
| **A2** | ~~Auto-apply the 380 "agrees" chains.~~ **DONE 2026-08-27 — and the lane has COMPLETED A TASK FOR THE FIRST TIME: `establish_ownership_history` completed **0 → 288**, open 545 → 257.** Migration `20260827130000` (applied live) + cron **244** (`lcc-a2-ownership-chain-apply`, 06:49 UTC, calling the SQL function directly so no Railway deploy is on the critical path). **322 historical facts written** across 280 owners / $579.9M; `agrees` 380 → 92; 5 current-owner acquisition dates filled; **0 of 322 facts read `is_current`**; `v_lcc_portfolio_ownership_conflict` 0 before and after. Reversible by batch tag (`lcc_a2_unapply_ownership_chains`) and the round trip was **run on real data before the real run** — a reversal path never executed is a claim, not a capability (P195). **Identity is `lcc_ownership_chain_name_key` (lower-then-strip, unambiguous-only, through `lcc_entity_survivor`) — `lcc_owner_strict_core` was TRIED and REJECTED here:** it drops 1-char tokens and sorts, so `BAMMF (8) LLC == BAMMF (3) LLC` and `F R M ASSOCIATES == G B A Associates`, and it matched 393 of 396 grantors, an implausibly clean number that is a bug signal (P182). **Found on the way: `r9_chain_connect` (cron 104) mints a prior-owner entity per chain name and attaches it to NOTHING** — 291 of the 331 resolved grantors are its output, retirable by the gov feeder's own predicate; A2 is the missing consumer. **Seed predicate closed and verified: 0 of the 288 completed properties can be re-seeded into this lane** (the FACT flips `owner_links` to ≥2, so the worklist suggests `trace_ownership_to_developer` instead — 284 moved there, a LIVE lane with 40 lifetime completions). 128 links stay blocked and named — **54 `ambiguous_entity` (LCC duplicate entities: `Duke Realty Limited Partnership` / `DUKE REALTY LIMITED PARTNERSHIP`; 41 of the 92 open tasks are blocked by this alone, so a P195-style merge unblocks them and the next nightly run applies them)**, 28 `repeat_transfer_unrepresentable`, 26 `placeholder`, 20 `no_entity`. Guard `test/ownership-chain-apply.test.mjs` (20 tests, mutation-verified red on **nine** breakages). Writeup: `docs/audits/A2_OWNERSHIP_CHAIN_APPLY_2026-08-27.md`. | ✅ | **the lane's first-ever completion** — and the 92 residue is now a named, ranked worklist rather than an undifferentiated queue |
...
| **A2a** | **UNBLOCKED by A2** — merge the duplicate entity pairs behind `v_lcc_ownership_chain_apply_blocked` where `blocked_reason='ambiguous_entity'` (54 links / **41 of the 92 still-open `agrees` tasks**). The view carries `rival_entity_names`; the population is case-variant pairs of one firm, i.e. exactly what P189/P195 work on. A2 never picks a winner — a byte-identical name is a merge question, not a licence. Once merged, **no new code is needed**: cron 244 applies them the same night. | 🟢 | 41 tasks, zero new machinery |
