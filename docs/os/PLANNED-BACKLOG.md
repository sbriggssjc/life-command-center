...
| N3 | **Prompt 188 — the Tier 0 confirm lane** and the residue P187 left. Nothing has ever been written to `owner_contact_pivot`, and that is still the right state. | 🟢 | `docs/claude-code/prompts/188-*.md` |
...
| # | Item | State | Notes |
|---|---|---|---|
| AC1 | **Tier 0 — link what we already know**, deterministically, by **email domain** ↔ owner identity; create the edge and promote the best into `owner_contact_pivot`. Value-gated; reuses the P161-gated reachability resolver. **Broker-only owners must fall THROUGH to acquisition, not be suppressed.** | 🟢 | Easterly alone yields 7 people, 2 with heavy correspondence |
