...
| **A2a** | **UNBLOCKED by A2** — merge the duplicate entity pairs behind `v_lcc_ownership_chain_apply_blocked` where `blocked_reason='ambiguous_entity'` (54 links across 50 tasks; **48 of the 92 still-open `agrees` tasks are blocked by ambiguity ALONE**, $210.6M). The view carries `rival_entity_names`; the population is case-variant pairs of one firm, i.e. exactly what P189/P195 work on. A2 never picks a winner — a byte-identical name is a merge question, not a licence. Once merged, **no new code is needed**: cron 244 applies them the same night. | 🟢 | 48 tasks, zero new machinery |
