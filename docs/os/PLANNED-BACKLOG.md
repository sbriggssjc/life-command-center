...
| # | Item | State | Source |
|---|---|---|---|
| V1 | **property-twin assist has not written since 2026-08-19.** P135 lifted the fixed 200-row window and dry-ran `fresh:895`, but live proposals are still 200 total / 0 in 7d. Confirm the nightly apply moves the count past 200. **⏱ RE-CONFIRMED STALLED 2026-08-26 22:5x — still exactly 200 / 0 in 7d.** Two consecutive nightly windows have now passed since the fix merged with no delta; this is no longer "awaiting verification", it is a second stall to diagnose. | 🔍 | re-measured 2026-08-26 (Cowork); `LOCAL-MODEL-LEVERAGE-MAP.md` §2 |
| V2 | **reachability-harvest has not written since 2026-08-13.** P136 added the negative marker + the evidence JOIN. `lcc_decisions` `reachability_harvest_review` = 4 total / 0 in 7d. Confirm by the proposal-count delta. **⏱ RE-CONFIRMED STALLED 2026-08-26 — still 4 / 0 in 7d, 13 days since the last write.** Same escalation as V1. | 🔍 | re-measured 2026-08-26 (Cowork); P136 |
...
| N8 | **⚠️ The sidebar channel has NEVER run the hardened extraction prompt — 0 hardened rows of 350 in 30 days, and it is the LARGEST producer (56%).** All seven Prompt-61 keys are structurally absent, `_provider` is unstamped, `stripNonSaleKeys` never runs. Ruled out already (do not re-walk): stale deploy — live `/version` **includes** the P61 commit; a second writer — exactly one insert site with both guards on the lines above it; a flow writing the table — none. **Leading untested hypothesis: the `seed_data` / extraction-race interaction** — the 96%-building-SF, 87%-cap profile is what a structured CoStar capture looks like, not a full-key LLM return. Needs **runtime evidence** (Railway logs for one sidebar intake end to end) → prompt 194. | 🔴 | `docs/audits/W53_INTAKE_CHANNEL_PROVENANCE_2026-08-26.md` |
