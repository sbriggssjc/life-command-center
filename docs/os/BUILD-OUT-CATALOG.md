...
|---|------|--------|-------|-----|-------|
| A1 | **Entity reconciliation** | 🔨 endpoint built, deploy pending | 🤖+🧑 | P2 | Specced + **built 2026-07-28**: `reconcile-entity` + `flagged-deals` endpoints (`entity-reconcile.js`) over two atomic DB fns (`reconcile_entity`, `list_flagged_open_deals`). Merge = repoint deals + move activity/edges + reversible tombstone. **DaVita Zapata already merged** (dup address). 6 open TB flagged remain → manual-pick via the endpoint (no SF FLS needed); closed 217 auto-reconcile later once A5b/addresses flow. |
