...
|---|------|--------|-------|-----|-------|
| A1 | **Entity reconciliation** | 📐 designed | 🤖+🧑 | P2 | **Specced 2026-07-28** (`entity-reconciliation-design.md`). 232 flagged, but only **15 open / 7 TB-owned** matter for live accuracy (217 are closed backfill). Two-tier: manual-pick the 7 now / auto-reconcile the rest by address once A5b lands. Merge op = repoint `bd_opportunities.entity_id`, move activity+edges, retire placeholder (reversible). One quick win: **DaVita Zapata** candidates are the same address twice → clean dup-merge. |
...
| A5b | **Populate asset property addresses** | 🔨 unblocked, next build | 🤖+🧑 | **P1 (unblocks A1+A5)** | **SF field CONFIRMED 2026-07-28**: Opportunity has `Property_Address__c` + `Property_Address_Line_1__c` (Formula Text). Build: map in `normalizeDeal`, use in `resolveDealEntity` to disambiguate candidates (fewer new flags), store on entity + `bd_opportunities`; add the field to the PA Get-records (🧑). Then A1 Tier-2 auto-reconcile + A5 address match both unlock. |
