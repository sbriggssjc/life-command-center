...
|---|------|--------|-------|-----|-------|
| A1 | **Entity reconciliation pass** | 📐 needs spec | 🤖+🧑 | **P1** | 232 deals in multi-asset cities created *flagged* ambiguous entities (`entities.metadata.ambiguous_resolution` = candidate list) instead of matching a real asset. Needs a review/merge workflow. **Design decision needed** (see Design Forks below). |
| A2 | **Contact-entity resolution backfill** | 🔨 | 🤖 | **P1** | Only 5,651/17,289 SF contacts resolve to a person entity — this caps both the roster and the email matcher. Backfill the resolver. |
...
| A4 | **Store SF `deal_name` on `bd_opportunities`** | 🔨 | 🤖 | P2 | Deal name currently lives only in `metadata`; promote to a column for cleaner joins/labels. |
| A5 | **Matcher recall signals** (address / escrow# / OM-PSA) | 📐 | 🤖 | P2 | Current matcher is tenant+city strong-signal (precise but misses e.g. Innovative Renal Care). Add more signals for recall without losing precision. |
| A6 | **Deal-party roster real source** | 📐 needs verify | 🤖+🧑 | P2 | SF OpportunityContactRole is a dead end for TB (0 rows on backbone). Verify the custom **`Deal_Participants__c`** object and/or `.md` dossier rosters as the true party source. **Design fork.** |
