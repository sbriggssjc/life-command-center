...
| A5 | **Matcher recall** | 🔨 reshaped | 🤖 | P2 | **Investigated 2026-07-28.** Original signals unbuildable: `entities.address` is **null for ~20/21 in-scope deals**, and escrow#/OM-PSA aren't stored either. Real lever: matcher requires the *full* tenant segment verbatim (misses "Innovative Renal Care MOB"). Fix = distinctive-token match (strip generic suffixes MOB/Dialysis/Center…, require core token + city). **Precision-sensitive → needs a validation/dry-count pass before it goes live.** |
| A5b | **Populate asset property addresses** (new, surfaced by A5) | 📐 | 🤖+🧑 | P2 | Assets lack street addresses — caps address-matching, geocoding, dedup. Source from the SF Opportunity property-address field (if present) into `entities.address` during sync. Unlocks A5's address signal + more. |
