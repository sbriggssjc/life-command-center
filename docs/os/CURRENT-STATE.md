...
| **Owner-role classification** | **`docs/architecture/owner-role-classification.md`** — designed, fully measured, **NOT built**. ⚠️ **MULTI-LABEL** (Scott: *"these categories can exist multiple iterations per one account"*) — **957 entities carry ≥2 roles, 772 of them `investor_owner` + `repeat_buyer`**, the population whose dual status decides seller-vs-buyer treatment. ✅ **Prompt staged: `C13b-owner-role-multilabel.md`** (C13 is superseded — single-role). ⛔ **`developer` is BUILT, not unbuilt** (§2e, `v_gov_owner_at_first_gen`, 2026-05-22) — do not build a second one. 🟠 **C18 blocks the part that matters most: `ownership_start_date` is 50.7%, so pacing is half unmeasurable.** |
