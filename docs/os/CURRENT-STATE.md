...
| **AI & OCR cost strategy** (local vs Microsoft vs Google) | **`docs/architecture/ai-and-ocr-cost-strategy.md`** — 🔴 **Tier 1 free OCR is designed, has a producer, and is NOT WIRED: `deps.freeOcr` has no server-side producer, so EVERY OCR call has always started at a paid tier** (OCR1). ~~🔴 The deed lane never tiers~~ ⚠️ **REFUTED 2026-09-02** (it tiers by default; the "325" predates DocAI). ✅ **OCR2 SHIPPED (#2050, migrations applied both domains, deploy `35528de9`)**: the drain now persists `extracted_data.document_text` provenance via `<dom>_merge_document_extracted_data`, the SINGLE writer of that column — the build found `deed-parser.js` had been REPLACING the whole jsonb, so provenance would have been destroyed on every deed. 507 historical deeds stay `unrecorded` by design. OCR2a/OCR2b filed. ⛔ **Microsoft is refuted: no batch-OCR API, Azure DI is separately metered, and IT blocks Azure AD app registrations** (OCR6). ✅ The **LLM** side is already largely local (`qwen2.5:14b` on GaryBuilt; 9 flags ON). ⚠️ **No pricing constant exists in code and the ~$1.50/1k rate is unverified** (OCR5). ⚠️ **MEASURED: cost is NOT the case — 51% of documents already extract free and total DocAI spend is 574 pages ≈ $0.86.** 🟢 **The case is that a local engine has NO PAGE CAP** — every hard problem in this arc was Google's 15/30-page limit, not money — plus confidentiality and resilience. 🟡 **OCR1: two workstation runs done 2026-09-02. With the self-agreement floor (93%), tesseract reads 80% (−13 pp) → §5 row 2: free pre-filter that removes the page cap, not a DocAI replacement. Surya/Paddle still ungraded → 👤 the box run decides OCR1b. ⚠️ The first box run (19:00 UTC) graded neither: the CPU paddle wheel was installed (`run_check` said "1 CPU") and surya's `SURYA_INFERENCE_BACKEND` was not switched — corrected steps on `OPERATOR-ACTIONS.md`.** ✅ **EXT1 SHIPPED 2026-09-02 (`de6daca`, DEPLOYED in `30eaced2`) and MEASURED — rent/date disagreements vs tesseract 2→0 / 4→0, but the self-rate stayed 89/80/80 because the model mislabels `basis`/`precision` on unambiguous quotes → ✅ EXT1b SHIPPED + deployed (`a013aea6`, #2068): `as_stated` parsed in code is the authority, labels the fallback; ✅ **MEASURED: rent 100%, expiration 100% on the self-control (floor 94%)** — the extractor no longer computes, defaults or mislabels; the residue is a DEFINITION (which rent line, which tenant name) → 👤 **EXT2**.** — the side finding is closed on the build side: `extractTenantFromLease` now takes a QUOTED rent (amount + the basis the lease states it on) and QUOTED dates (a date only at day precision) and does the annualization and any term arithmetic **in code**; a model `year1_rent` number is ignored whenever a quote is present. Consumer keys unchanged, no backfill. ⚠️ **The floor movement is PREDICTED, not measured — the sandbox has no OCR engine and no model, so `--control self` cannot run here; 👤 Scott's re-run on the same 10 arm-A docs is the verification, and `self_both_null` must be read beside the rate.** ⚠️ **Scope correction from the build: an uncapped engine removes the ACQUISITION ceiling only; `LEASE_TEXT_SLICE_CHARS` (90k) still caps CONSUMPTION at ~52pp median / ~33pp p90.** ✅ DOC18 live. **Next: Scott runs OCR1 (`OPERATOR-ACTIONS.md`), then OCR2.** |
