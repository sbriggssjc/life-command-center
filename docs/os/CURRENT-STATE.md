...
| **Document capture / OCR / deeds** (read this, not the five older files) | **`docs/architecture/document-capture-ocr-and-deeds.md`** — **now opens with a CURRENT STATE block; §0 below it is a dated worklog.** Live 2026-09-02: deeds **325/325** · drain **771 → 426** · **`bov_ready` 5 → 37** · **22+ OCR events, 100% DocAI, ZERO gpt-4o.** 🔴 **DOC14 — 40 documents get NO text at all; BLOCKED on Scott, and the gate is CONFIDENTIALITY (batch writes full executed-lease text to GCS as JSON), not cost (~$3).** ⚠️ **Re-sized ~2×: lease 17.0%, DD no longer 0, max 141 pages — which makes the unverified ~500pp async ceiling load-bearing.** ✅ **DOC13 answered — `retry_admitted:0` is correct.** 🔵 **DOC15 watch: retry-lane starvation.** ⛔ Do NOT widen cron 160 (DOC7). |
