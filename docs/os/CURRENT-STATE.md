...
| **Document capture / OCR / deeds** (read this, not the five older files) | **`docs/architecture/document-capture-ocr-and-deeds.md`** — **opens with a CURRENT STATE block; §0 below is a dated worklog.** Live 2026-09-02: deeds **325/325** · drain **771 → 419** · **`bov_ready` 5 → 39** · **100% DocAI, ZERO gpt-4o.** 🔴 **42 documents get NO text at all** (18 at 31–50pp, 24 at >50pp, max 141). ⛔ **DOC16 REFUTED** — the 30pp cap applies to a selection **only if it starts at page 1**, so a 30-page-only route drops ~40% of the consumer's 90,000-char window. 🟢 **DOC17: ONE API call decides between no-GCS and the full GCS build** — is a non-page-1 selection measured against the selection or the total? **Probe only.** 🔴 **DOC14 is the fallback** (GCS persistence + confidentiality; note Google already receives full lease bytes today). ⛔ Do NOT widen cron 160 (DOC7). |
