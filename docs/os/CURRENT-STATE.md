...
| **AI & OCR cost strategy** (local vs Microsoft vs Google) | **`docs/architecture/ai-and-ocr-cost-strategy.md`** — 🔴 **Tier 1 free OCR is designed, has a producer, and is NOT WIRED: `deps.freeOcr` has no server-side producer, so EVERY OCR call has always started at a paid tier** (OCR1). ~~🔴 The deed lane never tiers — all 325 deeds went to the 6–14× tier~~ ⚠️ **REFUTED 2026-09-02: the deed drain passes `ocrTiered: true` and no caller opts out; the "325 to gpt-4o" was a DATE artifact (154 of 185 dated deed extractions predate DocAI going live on 08-12). The real defect is that deed rows carry NO OCR provenance, so the tier mix is unauditable — OCR2 re-scoped to that.** ⛔ **Microsoft is refuted: no batch-OCR API, Azure DI is separately metered, and IT blocks Azure AD app registrations** (OCR6). ✅ The **LLM** side is already largely local (`qwen2.5:14b` on GaryBuilt; 9 flags ON). ⚠️ **No pricing constant exists in code and the ~$1.50/1k rate is unverified** (OCR5). ⚠️ **MEASURED: cost is NOT the case — 51% of documents already extract free and total DocAI spend is 574 pages ≈ $0.86.** 🟢 **The case is that a local engine has NO PAGE CAP** — every hard problem in this arc was Google's 15/30-page limit, not money — plus confidentiality and resilience. 🟡 **OCR1 first real run DONE 2026-09-02 — 15 real documents, tesseract only (surya→GaryBuilt, paddle missing `paddlepaddle`). Page-cap case MEASURED TRUE (141pp read in one pass, back-half clauses legible). Quality parity UNPROVABLE as built — ≥6 of 11 non-agreements are harness/model artifacts; OCR1c adds the self-agreement control, then re-run.** ⚠️ **Scope correction from the build: an uncapped engine removes the ACQUISITION ceiling only; `LEASE_TEXT_SLICE_CHARS` (90k) still caps CONSUMPTION at ~52pp median / ~33pp p90.** ✅ DOC18 live. **Next: Scott runs OCR1 (`OPERATOR-ACTIONS.md`), then OCR2.** |
