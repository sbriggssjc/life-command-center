...
| **Document capture / OCR / deeds** (read this, not the five older files) | **`docs/architecture/document-capture-ocr-and-deeds.md`** — live-verified 2026-09-01. ✅ **Byte-capture-at-ingest IS built and worked** (PR #1703/#1707): 90% of gov domain docs have bytes, **deeds 325/325 text**. ⚠️ **But there are TWO document stores, and the OTHER one is stuck: `fetchEligibleCreDocs` has a fixed 60-row window and no cursor, so 695 lease/dd/om docs never reach `bov-extract.js` while crons 167/169 return HTTP 200 every 30 min** (DOC1, prompt staged). ⛔ **Do NOT widen cron 160 to `doctype=all` — refuted, DOC7.** ⚠️ **`GovernmentProject` docs are STALE** (DOC2). |
