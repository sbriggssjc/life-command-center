...
| **Document capture / OCR / deeds** (read this, not the five older files) | **`docs/architecture/document-capture-ocr-and-deeds.md`** — live-verified 2026-09-01. ✅ **Byte-capture-at-ingest IS built and worked** (PR #1703/#1707): 90% of gov domain docs have bytes, **deeds 325/325 text**. ⚠️ **There are TWO document stores and conflating them is why this recurs.** The other one was stuck — a fixed 60-row window kept 695 lease/dd/om docs from ever reaching `bov-extract.js` while crons 167/169 returned HTTP 200 every 30 min. ✅ **DOC1 SHIPPED 2026-09-01** (oldest-first keyset scan + a dated negative marker for the byte-fetch failure that persisted nothing). ⚠️ **Merged is not running — run §7b, the standing status check, before quoting any number on that page.** ⛔ **Do NOT widen cron 160 to `doctype=all` — refuted, DOC7.** ⚠️ **`GovernmentProject` docs are STALE** (DOC2). |
