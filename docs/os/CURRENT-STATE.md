...
| **Document capture / OCR / deeds** (read this, not the five older files) | **`docs/architecture/document-capture-ocr-and-deeds.md`** — ✅ **DOC1 + DOC8/9/10 shipped, deployed, verified.** Drain reaches id 2 (undrained 695 → **639**); **0 rows covered-and-thin**; **`bov_ready` 5 → 11**; deeds 325/325. ✅ **The gpt-4o escalation is GONE — 9 of 9 OCR events on DocAI since redeploy** and `over_docai_page_cap` fires (2 rows, 31/39 pages). ⚠️ **But that is the MARKER's doing, not the cap raise's — the 16–30 band has still had ZERO successful OCR, so the 15 → 30 raise is UNEXERCISED (DOC12).** 🔵 **DOC13: retry re-admission, time-gated to 2026-09-02 ~16:30 UTC.** ⛔ Do NOT widen cron 160 (DOC7). |
