...
| **BD ranking / the priority queue** (the chain's LAST hop — read this, not the C4/C5 audits) | **`docs/architecture/bd-ranking-and-priority-queue.md`** — one door into C4→C5→C6: the role gate, the `buyer` category error, broker assignment, and the four traps. ✅ **C6 SHIPPED 2026-08-29 · C8 SHIPPED 2026-08-31** (call sheet 80 → 126 rows; ✅ **C8c FIXED 2026-08-31 as C10** — the sheet renders real names and portfolio values, count held at 126; ✅ **C10b FIXED 2026-08-31 as C11** — it now states the BASIS on which each person is the contact (the recorded owner→contact edge role: `prospecting_contact` 58 · `institution_decision_maker` 35 · `manager` 15 · **`works_at` 12, labelled *association only* per P161** · `decision_maker` 1 · **no edge 5**), plus employer corroboration as an ADDITIVE POSITIVE — ⚠️ **22 of 113, not the 16 previously recorded**, and still a lower bound; nothing filters or ranks on it. New 🔴 **C11a/C11b/C11c**). C6 — the seller-side bands gate on *current holding + reachability*, not on `owner_role`; **P1 74→149 · P2 32→95 · P3 61→163 · P8 76→213, 303 owners, all callable**. ⚠️ **C4a (the bucket/pitch) and C4b (`user_owner`, still gating 761 rows) remain open** |
