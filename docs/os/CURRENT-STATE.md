...
| **Document capture / OCR / deeds** (read this, not the five older files) | **`docs/architecture/document-capture-ocr-and-deeds.md`** — capture → storage → OCR → consumption, live-verified 2026-08-31. ✅ **Byte-capture-at-ingest IS built and live** (PR #1703/#1707); 1,057 of 1,177 gov docs have bytes. ⚠️ **But cron 160 drains only `doctype=deed`: deeds are 325/325 text, everything else is 0%, and 732 documents with bytes have no scheduled pass — including 119 leases** (DOC1). ⚠️ **`GovernmentProject` docs are STALE and say the crons are off and a residential-egress CoStar fetcher is needed — both false** (DOC2). |
