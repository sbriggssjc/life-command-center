...
| **Document capture / OCR / deeds** (read this, not the five older files) | **`docs/architecture/document-capture-ocr-and-deeds.md`** — ✅ **DOC1 + DOC8/9/10 shipped 2026-09-01.** CRE drain reaches id 2 (undrained 695 → 682); DocAI cap **confirmed 15 → 30 on behaviour**; **tier split flipped — today 3 DocAI at 601–7,572 chars vs 2 gpt-4o at 116/211**; DOC10 backfill 12 rows, `bov_ready` 7 → 4 (**down IS the fix**). Deeds 325/325. 🔴 **DOC11 — the Railway redeploy has NOT run**, so DOC9/DOC10's JS is inert and **doc 24 landed `needs_ocr=false` at 211 chars four minutes after DOC10 shipped.** ⛔ Do NOT widen cron 160 (DOC7). |
