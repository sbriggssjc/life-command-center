...
| **Document capture / OCR / deeds** (read this, not the five older files) | **`docs/architecture/document-capture-ocr-and-deeds.md`** — **opens with a CURRENT STATE block.** Live 2026-09-02: deeds **325/325** · drain **771 → ~419** · **`bov_ready` 5 → 39** · **100% DocAI, ZERO gpt-4o.** ✅ **DOC17 settled the decider: the DocAI sync page cap is measured against the SELECTION, not the document — 30pp/call from page 1 (imageless), 15pp/call anywhere else.** 🟢 **DOC18 staged: a three-call sync route clears all 42 over-cap documents for ~$3.30 with NO GCS, no IAM and no confidentiality decision.** 👤 **DOC14 should probably be closed — Scott's call.** ⛔ Do NOT widen cron 160 (DOC7). |
