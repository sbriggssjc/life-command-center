...
| **Document capture / OCR / deeds** (read this, not the five older files) | **`docs/architecture/document-capture-ocr-and-deeds.md`** — ✅ **DOC1 + DOC8/9/10 shipped and verified; DOC12 closed.** Drain 695 → **615**, `bov_ready` 5 → **13**, **zero gpt-4o escalations in 14 OCR events**, deeds 325/325. ⚠️ **But the cap raise fixed almost nothing — the 16–30 band holds ONE document in the whole corpus; the MARKER closed the escalation.** 🔴 **DOC14: 5 documents at 31–57 pages now get NO text at all** — marked, invisible, never extracted, and almost certainly the full executed leases BOV extract most wants. 🔵 **DOC13 (retry re-admission) is time-gated to 2026-09-02 ~16:30 UTC and has NOT run.** ⛔ Do NOT widen cron 160 (DOC7). |
