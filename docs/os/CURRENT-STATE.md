...
| **BD ranking / the priority queue** (the chain's LAST hop — read this, not the C4/C5 audits) | **`docs/architecture/bd-ranking-and-priority-queue.md`** — one door into C4→C5→C6: the role gate, the `buyer` category error, broker assignment, and the four traps. ✅ **C6 SHIPPED 2026-08-29 · C8 SHIPPED 2026-08-31** (call sheet 80 → 126 rows; ⚠️ **C8c: every row still renders "Unknown … rent unknown"** — prompt C10 written, not yet run). C6 — the seller-side bands gate on *current holding + reachability*, not on `owner_role`; **P1 74→149 · P2 32→95 · P3 61→163 · P8 76→213, 303 owners, all callable**. ⚠️ **C4a (the bucket/pitch) and C4b (`user_owner`, still gating 761 rows) remain open** |
