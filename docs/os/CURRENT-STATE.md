...
| **Document capture / OCR / deeds** (read this, not the five older files) | **`docs/architecture/document-capture-ocr-and-deeds.md`** — ✅ **DOC1 shipped 2026-09-01 and the CRE drain is live**: reaches document id 2, undrained 695 → 691, `bov_ready` 5 → 6. Deeds 325/325. ✅ **DOC8/9/10 shipped 2026-09-01 (§0d)**: `docai-ocr` **v24 sets `imagelessMode`, cap 15 → 30 — CONFIRMED LIVE 16:00:35 UTC, Google's own reported limit moved to 30** (deployed to LCC Opps; a TOP-LEVEL ProcessRequest boolean, verified against the live v1 discovery doc — NOT `processOptions.ocrConfig`), above 30 a named `over_docai_page_cap` marker instead of a silent gpt-4o fall-through, the spend counter counts the ENGINE unconditionally (⚠️ **`ocr_by_engine` REMOVED — read `ocr_docs_by_engine` / `ocr_pages_unknown`**), and a thin OCR result now writes DOC1's dated marker. **Backfill 12 rows / 9 properties; `bov_ready` 7 → 4 — DOWN is the fix working.** ⚠️ **The JS half ships on the Railway redeploy of PR #1995**, so the 12 thin rows are hidden but not yet retried, and the post-DOC8 tier split is not yet measurable. ⛔ Do NOT widen cron 160 (DOC7). |
