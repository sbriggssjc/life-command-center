...
| **Document capture / OCR / deeds** (read this, not the five older files) | **`docs/architecture/document-capture-ocr-and-deeds.md`** — **now opens with a CURRENT STATE block; §0 below it is a dated worklog.** Live 2026-09-02: deeds **325/325** · drain **771 → 426** · **`bov_ready` 5 → 37** · **22+ OCR events, 100% DocAI, ZERO gpt-4o.** 🟢 **DOC16 staged and probably removes the need for GCS: `bov-extract` truncates lease text at 90,000 chars ≈ 50 pages, so 24 of the 40 over-cap documents are 51–141pp and a full extract delivers DISCARDED text.** Route is two sync calls (pp 1–30, 31–50) concatenated — ⚠️ rests on whether the 30pp cap applies to the SELECTION or the TOTAL. 🔴 **DOC14 is the fallback, blocked on Scott (GCS persistence-at-rest — note Google already receives full lease bytes today).** ⚠️ **Re-sized ~2×: lease 17.0%, DD no longer 0, max 141 pages — which makes the unverified ~500pp async ceiling load-bearing.** ✅ **DOC13 answered — `retry_admitted:0` is correct.** 🔵 **DOC15 watch: retry-lane starvation.** ⛔ Do NOT widen cron 160 (DOC7). |
