> 🚨 **SUPERSEDED/STALE (DOCMAP3, 2026-09-24; moved from `SPEC_sos_direct_scraper.md`):** the LCC side does carry the `llc-research.js` `SOS_DIRECT_ADAPTERS` framework and `STAGE_SOS` (gated by `W9_1_SOS_DIRECT`, still off). The fetcher and residential proxy live in government-lease (`sos-proxy/`, `CLAUDE.md` §25). The FL Sunbiz bulk mirror is backlog `A5g`.
