> 🚨 **SUPERSEDED/STALE (DOCMAP3, 2026-09-24; moved from `docs/CONTACTS_SPLIT_BRAIN_CUTOVER_RUNBOOK.md`):** gov copy: measured 2026-09-24, gov `unified_contacts` / `contact_change_log` / `contact_merge_queue` have RLS on and no anon grants (step 5 done). But the gov copy is NOT frozen: it still takes new rows (see backlog `CONTACTS-GOV-WRITER`).
