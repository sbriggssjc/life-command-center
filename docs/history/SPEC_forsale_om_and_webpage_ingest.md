> 🚨 **SUPERSEDED/STALE (DOCMAP3, 2026-09-24; moved from `SPEC_forsale_om_and_webpage_ingest.md`):** fully built. Part A is `STAGE_OM_VIA_TAB` / `marketing_brochure`, B1 is `listing_external_urls` in `sidebar-pipeline.js`, and B2 is `listing_web_pages` + `/api/listing-page-crawl`. The only open item is the `LISTING_PAGE_PROACTIVE_EXTRACT` flag (CURRENT-STATE). See `docs/architecture/costar-sidebar-capture-pipeline.md` and `om_intake_pipeline.md`.
