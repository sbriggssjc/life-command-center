> 🚨 **SUPERSEDED/STALE (DOCMAP3, 2026-09-24; moved from `docs/PHASE1_SALESFORCE_BRIDGES.md`):** no `sf.*` bridge was ever seeded (measured 2026-09-24: `connector_bridges` holds only `outlook.messages`; `salesforce_activity_log` = 0 rows). Routing is `server.js` on Railway, not `vercel.json`/Vercel Cron. Salesforce data reaches LCC by other paths; see `docs/architecture/connectivity-and-open-threads.md`. The activate-or-retire decision is backlog `BRIDGES-DORMANT`.
