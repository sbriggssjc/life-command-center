> 🚨 **SUPERSEDED/STALE (DOCMAP3, 2026-09-24; moved from `docs/PHASE1_5_USER_MAPPINGS_AND_WRITEBACK.md`):** `external_user_mappings` does not exist on LCC Opps and `sf.touchpoint.log` was never seeded (measured 2026-09-24), so `/api/sf-write` cannot work as described. Env goes in Railway, not Vercel. Inbound source-user ids resolve through `api/_shared/source-user-id.js::resolveSourceUserId` (P116). The activate-or-retire decision is backlog `BRIDGES-DORMANT`.
