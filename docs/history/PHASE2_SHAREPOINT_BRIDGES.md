> 🚨 **SUPERSEDED/STALE (DOCMAP3, 2026-09-24; moved from `docs/PHASE2_SHAREPOINT_BRIDGES.md`):** the `sharepoint.properties.*` bridges were never seeded (`sharepoint_documents` = 0 rows, measured 2026-09-24). SharePoint documents arrive via the folder feed (migrations `20260718*_lcc_phase2_folder_feed*`) and the doc-bytes backfill in root `CLAUDE.md`. Routing is Railway `server.js`, not `vercel.json`.
