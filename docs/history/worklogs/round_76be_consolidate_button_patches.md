> 🚨 **SUPERSEDED/STALE (DOCMAP3, 2026-09-24; moved from `docs/round_76be_consolidate_button_patches.md`):** the patches were applied long ago, and the Vercel rewrite no longer exists (routing is `server.js`). ⚠️ The Consolidate handler still calls the non-reversible merges directly: gov `gov_merge_property` now RAISES, so the gov button fails. See backlog `CONSOLIDATE-REVERSIBLE`.
