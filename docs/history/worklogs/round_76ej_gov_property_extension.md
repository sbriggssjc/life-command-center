> 🚨 **SUPERSEDED/STALE (DOCMAP3, 2026-09-24; moved from `docs/round_76ej_gov_property_extension.md`):** `sourceSystem='gov_db'` and `entities.domain='government'` are banned spellings (canonical `gov`), so the verification SQL here (`domain = 'government'`) returns nothing. See root `CLAUDE.md` Domain-DB invariants.
