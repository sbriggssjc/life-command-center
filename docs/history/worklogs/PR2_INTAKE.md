> 🚨 **SUPERSEDED/STALE (DOCMAP3, 2026-09-24; moved from `docs/PR2_INTAKE.md`):** PR #2's Tier-1 scope (source-summary keys, GSA link comparison, link_pick confidence gate) and O2 (sale-link resolver city query) are built in `gov.js`; this is a closed 2026-05 round note, not an open plan. Its open Tier-2 idea (generalize the Sale-Link Resolver to every `link_pick` reason) is filed as backlog `DOCMAP3-govlinkpick`.
