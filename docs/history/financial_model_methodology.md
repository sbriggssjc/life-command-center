> 🚨 **SUPERSEDED/STALE (DOCMAP3, 2026-09-24; moved from `docs/financial_model_methodology.md`):** revenue is no longer patients × 156 × rate. It is HCRIS treatments × payer-weighted rate (`dialysis_econ_reconciled_v1`, Dialysis `CLAUDE.md`), and the ~$443/tx blended validation is obsolete (live code ~$357). Truth: `docs/architecture/dialysis-economics-and-medicare-data.md`. The Lease Risk Score section may still describe live `detail.js` logic; read the code before relying on it.
