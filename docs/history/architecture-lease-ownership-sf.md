> 🚨 **SUPERSEDED/STALE (DOCMAP3, 2026-09-24; moved from `docs/architecture-lease-ownership-sf.md`):** early design, since implemented in `sidebar-pipeline.js` (`LENDER_PATTERN`, `crossReferenceSalesforce`, lease chains). Its counts are obsolete, Vercel is retired (Railway), and fuzzy-name identity matching is banned; see `docs/architecture/data-quality-lease-and-owner.md` and `property-owner-subsystem.md`.
