> 🚨 **SUPERSEDED/STALE (DOCMAP3, 2026-09-24; moved from `docs/PHASE4_CADENCE_ENGINE.md`):** `cadence_alerts` and `v_contact_engagement` do not exist on LCC Opps, and no cron calls `/api/cadence-tick` (measured 2026-09-24), so this alerter never ran. Live cadence truth is `docs/architecture/cadence-engine.md` and root `CLAUDE.md` (P112 BD-signal gate).
