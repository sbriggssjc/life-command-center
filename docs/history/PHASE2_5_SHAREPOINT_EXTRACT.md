> 🚨 **SUPERSEDED/STALE (DOCMAP3, 2026-09-24; moved from `docs/PHASE2_5_SHAREPOINT_EXTRACT.md`):** never activated: it depends on the Phase 2 SharePoint bridges, which were never seeded (`sharepoint_documents` = 0 rows, measured 2026-09-24), and the `PA_SP_EXTRACT_URL` flow was never built. SharePoint documents arrive via the folder feed and the doc-bytes backfill (root `CLAUDE.md`). Routing is Railway `server.js`.
