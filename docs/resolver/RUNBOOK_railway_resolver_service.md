...
| `RESOLVER_STORAGE_KEY` | *(read-only Storage token)* | **Read-only.** Used solely to download the corpus JSONL for `/train`. Do NOT give it a service-role write key — this service must not be able to write. |
