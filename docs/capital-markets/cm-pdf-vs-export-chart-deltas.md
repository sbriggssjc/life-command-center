> 🚨 **SUPERSEDED/STALE (DOCMAP3, 2026-09-24; moved from `docs/cm-pdf-vs-export-chart-deltas.md`):** this is a May-2026 planning inventory. Many of its unticked items have since shipped, and gov cap-by-term is now 3 buckets (government-lease `CLAUDE.md` §28). The live chart catalog is `public/reports/cm_chart_catalog.json` + `api/_shared/cm-excel-export.js`. The "Calibri" typography note conflicts with the Northmarq brand (Futura PT / Arial; `public/reports/cm-brand.json`).
